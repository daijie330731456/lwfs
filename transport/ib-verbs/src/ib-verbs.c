...
#ifndef WEIWEI_20120716
#include <string.h>
#include <errno.h>
#endif

#ifndef JJH /* add unref trans to list */
extern void compute_checksum_forward (char *, size_t , uint32_t *);
int			unref_max;
int			unref_num;
pthread_mutex_t 	unref_trans_lock;
struct	list_head	unref_trans_list;
void
transport_add_unref(ib_verbs_private_t *priv)
{
	ib_verbs_private_t	*del = NULL;
	pthread_mutex_lock(&unref_trans_lock);
	list_add_tail(&priv->priv_list,&unref_trans_list);
	unref_num ++;
	if ( unref_num > unref_max ) {
		del = list_entry(&unref_trans_list.next,ib_verbs_private_t, priv_list);
		list_del_init(&del->priv_list);
		unref_num --;
	}
	pthread_mutex_unlock(&unref_trans_lock);
	if ( del != NULL ) {
		transport_unref(priv->priv_trans);
	}
	return;
}
...
ib_verbs_new_post (ib_verbs_device_t *device, int32_t len)
{
        ...
        if (!post){
		// weiwei added at 2010-08-23
		gf_log ("transport/ib-verbs",
			GF_LOG_ERROR,
			"can not calloc memory!");
                return NULL;
	}
	...
	// weiwei added at 2010-04-13
	memset(post->buf, 0 ,len);
		...
		gf_log ("transport/ib-verbs",
			GF_LOG_ERROR,
			"ibv_reg_mr failed:%s.buf=%#lx,buf_size=%#lx",strerror (errno),post->buf,post->buf_size);
                free (post->buf);
                free (post);
                return NULL;
        }
...
__ib_verbs_ioq_churn_entry (ib_verbs_peer_t *peer, ib_verbs_ioq_t *entry)
{
		...
		// weiwei add at 2010-08-23
		if(!post){
			sleep(2);		// wait a while,then can OK?
                        post = ib_verbs_new_post (device, 
                                                  (options->send_size + 2048));
                        return -1;
		}
...
#ifdef JJH	/* set cksum for ib pack */
		uint32_t tmpchk = 0;		
		ib_verbs_header_t *header=NULL;
		header = (ib_verbs_header_t *)post->buf;
		header->cksum = 0;
		compute_checksum_forward(post->buf, len, &tmpchk);
		header->cksum = tmpchk;
#endif
...
#ifndef JJH
	entry->header.cksum = 0;   /* initial to 0 */
#endif
...
#ifndef JJH	/* check cksum of ib pack */
	ckbak = header->cksum;
	if( ckbak !=0 ){
		header->cksum = 0;
		compute_checksum_forward(copy_from,data_len,&checksum);
		if ( ckbak != checksum ) {
			gf_log ("Recieve", GF_LOG_NORMAL,
				"recv_ck[%u],com_ck[%u],recieve_len = %d.",ckbak,checksum,data_len);
			ret = -1;
			goto err;
...
#ifndef WEIWEI_20120413
                                         options->recv_count * 32,
#else
                                         options->recv_count * 2,	// enlarge recv_count of CQ,in case of overflow.
#endif
                                         device,
                                         device->recv_chan,
                                         0);
                          ...
                          int32_t qp_num)
{
        ...
        if (ent->qp_num != qp_num) {
         	...
         	gf_log ("transport/ib-verbs",
                        GF_LOG_ERROR,
              		"JJH: unregister_peer qpnum[%d] not found!!!!!",
              		qp_num);
		device->fail_qp_num ++;
#endif
                pthread_mutex_unlock (&qpreg->lock);
...
__ib_verbs_destroy_qp (transport_t *this)
{
	...
	int ret = 0;
		...
		// weiwei added at 2010-10-15
		gf_log("transport/ib-verbs", GF_LOG_TRACE,
			"ibv_destroy_qp(%d).",priv->peer.qp->qp_num);

                ret = ibv_destroy_qp (priv->peer.qp);
#ifndef JJH
		if(ret != 0 )
			gf_log("transport/ib-verbs", GF_LOG_ERROR,
				"JJH:ibv_destroy_qp failed(%s).",strerror (errno));

#else
		if(ret != 0 )
			gf_log("transport/ib-verbs", GF_LOG_TRACE,
				"ibv_destroy_qp failed(%s).",strerror (errno));
#endif
        }
        priv->peer.qp = NULL;
...
ib_verbs_create_qp (transport_t *this)
{
		...
		pthread_mutex_lock(&device->qpreg.lock);
                gf_log ("JJH",
                        GF_LOG_ERROR,
                        "Could not create QP, created[%d],fail[%d],reg[%d],unreg[%d]",
			device->create_qp_num, device->fail_qp_num,
			device->reg_qp_num, device->unreg_qp_num);
		pthread_mutex_unlock(&device->qpreg.lock);
                ...
                gf_log ("transport/ib-verbs",
                        GF_LOG_ERROR,
                        "JJH: failed to modify QP to INIT state");
		pthread_mutex_lock(&device->qpreg.lock);
		device->fail_qp_num ++;
		pthread_mutex_unlock(&device->qpreg.lock);
#else
	...
	// weiwei added at 2010-10-19
	gf_log ("transport/ib-verbs",GF_LOG_TRACE,"qp_num=%d.",peer->qp->qp_num);
out:
        if (ret == -1)
...
ib_verbs_create_posts (transport_t *this)
{
        ...
        ret =  __ib_verbs_create_posts (this, options->send_count,
                                        ...
                                        &device->sendq);
        if (!ret)
#ifndef WEIWEI_20130204
                ret =  __ib_verbs_create_posts (this, options->recv_count*4,
                                                options->recv_size,
                                                &device->recvq);
#else
...
ib_verbs_connect_qp (transport_t *this)
{
        ...
        attr.timeout        = 22;	// WEIWEI changed it from 14 to 22 at 20120405
        attr.retry_cnt      = 6;	// WEIWEI changed it from 7 to 6 at 20120405
        attr.rnr_retry      = 6;	//  WEIWEI changed it from 7 to 6 at 20120405
...
ib_verbs_recv_completion_proc (void *data)
{
...
#ifndef WEIWEI_20120711
	int flag = 0;
	struct pollfd recv_pollfd = {0,};

	/* WEIWEI: make sure fnctl is success.If failed,do not do poll operation. */
	flag = fcntl (chan->fd, F_GETFL);
	fcntl (chan->fd, F_SETFL, flag|O_NONBLOCK);

	recv_pollfd.fd = chan->fd;
	...
	recv_pollfd.revents = 0;
        ...
        while (1) {
#ifndef WEIWEI_20120711
		ret = poll (&recv_pollfd, 1, 15000);	// WEIWEI_20120711 timoutout is 15 seconds.
		if (ret < 0) {
			gf_log ("transport/ib-verbs", GF_LOG_ERROR,
				"Poll fd failed, reason(%s),continue.",strerror(errno));
			continue;
		} else if (ret == 0) {
			continue;
		}
...
#ifndef WEIWEI_20120711
	int flag = 0;
	struct pollfd send_pollfd = {0,};

	flag = fcntl (chan->fd, F_GETFL);
	fcntl (chan->fd, F_SETFL, flag|O_NONBLOCK);

	send_pollfd.fd = chan->fd;
	send_pollfd.events = POLLIN;
	send_pollfd.revents = 0;
#endif
        
        while (1) {
#ifndef WEIWEI_20120711
		ret = poll (&send_pollfd, 1, 15000);	// WEIWEI_20120711 timeout is 15 seconds.
		if (ret < 0) {
			gf_log ("transport/ib-verbs", GF_LOG_ERROR,
				"Poll comp_chan'sfd failed,reason(%s),continue.",strerror(errno));
			continue;
		} else if  (ret == 0) {
			continue;
		}
...
#ifndef WEIWEI_20130204
        options->send_size = this->xl->ctx->page_size; /* 128KB */
        options->recv_size = this->xl->ctx->page_size; /* 128KB */
#else
        options->send_size = this->xl->ctx->page_size * 4; /* 512 KB */
        options->recv_size = this->xl->ctx->page_size * 4; /* 512 KB */
#endif
        options->send_count = 32;
#ifndef JJH	/* enlarge cq, ib_dev will reset when cq full */
        options->recv_count = 1024;
#else
        options->recv_count = 32;
#endif
...
#ifndef JJH	/* debug qp full */
		trav->create_qp_num = 0;
		trav->fail_qp_num = 0;
		trav->reg_qp_num = 0;
		trav->unreg_qp_num = 0;
#endif
...
#ifndef JJH
	INIT_LIST_HEAD (&priv->priv_list);
	priv->priv_trans = this;
	priv->handshake_ret = 0;
        pthread_mutex_init (&priv->priv_mutex, NULL);
#endif
...
ib_verbs_handshake_pollin (transport_t *this)
{
		...
		// weiwei added at 2010-10-15
		gf_log ("MEN-pollin", GF_LOG_TRACE,
			"IB_VERBS_HANDSHAKE_COMPLETE(%d).",priv->tcp_connected);
                return -1;
        }

        pthread_mutex_lock (&priv->write_mutex);
        {
		// weiwei added for connect and disconnect bug 
		if(priv->tcp_connected != 1)
			gf_log ("MEN-pollin", GF_LOG_ERROR,
				"tcp_connected=%d.",priv->tcp_connected);
				...
				// weiwei added for connect and disconnect bug 
				if(priv->handshake.incoming.count != 1){
					gf_log ("MEN-pollin",
						GF_LOG_ERROR,
						"MEN IB_VERBS_HANDSHAKE_RECEIVING_DATA,"
						"count=%d,pending_count=%d.",
...
unlock:
	...
	// weiwei added at 2010-10-15
	gf_log("ibverbs",GF_LOG_TRACE,"ret=%d, state=%d.",ret,priv->handshake.incoming.state);
...
ib_verbs_handshake_pollout (transport_t *this)
{
		...
		// weiwei added at 2010-10-15
		gf_log ("MEN-pollout", GF_LOG_TRACE,
			"IB_VERBS_HANDSHAKE_COMPLETE(%d).",priv->tcp_connected);
                return 0;
        }
		...
		// weiwei added for connect and disconnect bug 
		if(priv->tcp_connected != 1)
			gf_log ("MEN-pollout", GF_LOG_ERROR,
				"tcp_connected=%d.",priv->tcp_connected);
				...
				// weiwei added for connect and disconnect bug 
				if( priv->handshake.outgoing.count != 1 ){
					gf_log ("MEN-pollout",
						GF_LOG_ERROR,
...
ib_verbs_handshake_pollerr (transport_t *this)
{
		...
		if (  priv->handshake.incoming.state != IB_VERBS_HANDSHAKE_COMPLETE ) {
			priv->tcp_connected = priv->connected = 0;
			need_unref = 0; //let this memory leak till we find a better way to unref?
			priv->handshake_ret = EBADF;
			gf_log ("transport/ib-verbs", GF_LOG_ERROR,
				"%s: pollerr  in (peer: %s) "
				"while handshake is going",
	...
	} else if ( priv->handshake_ret != 0 ) {
		transport_add_unref(priv);
	}
#else
        if (need_unref)
                transport_unref (this);
                        ...
                        int poll_in, int poll_out, int poll_err)
{
	...
	// weiwei added at 2010-10-15
	gf_log("transport/ib-verbs",GF_LOG_TRACE,
		"priv->tcp_connected=%d,in=%d,out=%d,err=%d.",
		priv->tcp_connected,poll_in,poll_out,poll_err);
		...
		// weiwei added at 2010-10-15
		gf_log ("transport/ib-verbs", GF_LOG_TRACE,"handshake_pollout.");
		...
		// weiwei added at 2010-10-15
		gf_log ("transport/ib-verbs", GF_LOG_TRACE,"handshake_pollin.");
                ret = ib_verbs_handshake_pollin (this);
        }

        if (ret < 0 || poll_err) {
		// weiwei added at 2010-10-15
		gf_log ("transport/ib-verbs", GF_LOG_TRACE,"handshake_poller.");
...
#ifndef WEIWEI_20110226
	struct timeval timeout;
	timeout.tv_sec = 120; 
	timeout.tv_usec = 0;
	
	setsockopt (priv->sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof (timeout));
	setsockopt (priv->sock, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof (timeout));
#endif
        if (bind (priv->sock,
                  (struct sockaddr *)&sockaddr,
                  sockaddr_len) != 0) {
...
init (transport_t *this)
{
	...
	unref_max = 64;
	unref_num = 0;
	pthread_mutex_init(&unref_trans_lock,NULL);
	INIT_LIST_HEAD(&unref_trans_list);
#endif
        return 0;
}
