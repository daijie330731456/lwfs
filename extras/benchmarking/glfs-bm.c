...
struct state {
        ...
        char need_mode_liblwfsclient:1;
            ...
            struct argp_state *_state)
{
        ...
        case 'm':
                if (strcasecmp (arg, "posix") == 0) {
                        ...
                        state->need_mode_liblwfsclient = 0;
                } else if (strcasecmp (arg, "liblwfsclient") == 0) {
                        state->need_mode_posix = 0;
                        state->need_mode_liblwfsclient = 1;
                } else if (strcasecmp (arg, "both") == 0) {
                        state->need_mode_posix = 1;
                        state->need_mode_liblwfsclient = 1;
                } else {
...
do_mode_posix_iface_xattr_write (struct state *state)
{
        ...
        for (i=0; i<state->count; i++) {
                ...
                sprintf (key, "lwfs.file.%s.%06ld", bname, i);
...
int
do_mode_liblwfsclient_iface_fileio_write (struct state *state)
{
        long int i;
        int ret = -1;
        char block[state->block_size];
        ...
        for (i=0; i<state->count; i++) {
                ...
                fd = lwfs_glh_open (state->liblwfsclient_context,
                ...
                ret = lwfs_write (fd, block, state->block_size);
                if (ret == -1) {
                        fprintf (stderr, "lwfs_write(%s) => %s\n", filename, strerror (errno));
                        lwfs_close (fd);
                        break;
                }
                lwfs_close (fd);
...
int
do_mode_liblwfsclient_iface_fileio_read (struct state *state)
{
        long int i;
        int ret = -1;
        char block[state->block_size];
        ...
        for (i=0; i<state->count; i++) {
                ...
                if (fd == 0) {
                        fprintf (stderr, "lwfs_glh_open(%s) => %s\n",
                                 filename, strerror (errno));
                        break;
                }
                ret = lwfs_read (fd, block, state->block_size);
                if (ret == -1) {
                        fprintf (stderr, "lwfs_read(%s) => %s\n", filename,
                                 strerror (errno));
                        lwfs_close (fd);
                        break;
                }
...
int
do_mode_liblwfsclient_iface_fileio (struct state *state)
{
        if (state->need_op_write)
                MEASURE (do_mode_liblwfsclient_iface_fileio_write, state);

        if (state->need_op_read)
                MEASURE (do_mode_liblwfsclient_iface_fileio_read, state);
...
int
do_mode_liblwfsclient_iface_xattr_write (struct state *state)
{
        long int i;
        int ret = -1;
        char block[state->block_size];
        ...
        for (i=0; i<state->count; i++) {
                ...
                ret = lwfs_glh_setxattr (state->liblwfsclient_context,
                                              dname, key, block,
                                              state->block_size, 0);

                if (ret < 0) {
                        fprintf (stderr, "lwfs_glh_setxattr (%s, %s, %p) "
                                 "=> %s\n", dname, key, block,
                                 strerror (errno));
                        break;
                }
...
int
do_mode_liblwfsclient_iface_xattr_read (struct state *state)
{
        long int i;
        int ret = -1;
        char block[state->block_size];
        ...
        for (i=0; i<state->count; i++) {
                ...
                ret = lwfs_glh_getxattr (state->liblwfsclient_context,
                                              dname, key, block,
                                              state->block_size);

                if (ret < 0) {
                        fprintf (stderr, "lwfs_glh_getxattr (%s, %s, %p) "
                                 "=> %s\n", dname, key, block,
                                 strerror (errno));
                        break;
                }
...
int
do_mode_liblwfsclient_iface_xattr (struct state *state)
{
        if (state->need_op_write)
                MEASURE (do_mode_liblwfsclient_iface_xattr_write, state);

        if (state->need_op_read)
                MEASURE (do_mode_liblwfsclient_iface_xattr_read, state);
...
int
do_mode_liblwfsclient (struct state *state)
{
        lwfs_init_params_t ctx = {
                .logfile = "/dev/stderr",
                .loglevel = "error",
                .lookup_timeout = 60,
                .stat_timeout = 60,
	...
	ctx.specfile = state->specfile;
        if (state->specfile) {
                state->liblwfsclient_context = lwfs_init (&ctx, 1);

                if (!state->liblwfsclient_context) {
                        fprintf (stdout, "Unable to initialize lwfs "
                                 "context, skipping liblwfsclient mode\n");
                        return -1;
                }
        } else {
                fprintf (stdout, "lwfs volume specification file not "
                         "provided, skipping liblwfsclient mode\n");
                return -1;
        }

        if (state->need_iface_fileio)
                do_mode_liblwfsclient_iface_fileio (state);

        if (state->need_iface_xattr)
                do_mode_liblwfsclient_iface_xattr (state);
...
{
        if (state->need_mode_liblwfsclient)
                do_mode_liblwfsclient (state);
...
static struct argp_option options[] = {
        ...
        {"mode", 'm', "MODE", 0,
         "POSIX|LIBLWFSCLIENT|BOTH - defaults to POSIX"},
        {"block", 'b', "BLOCKSIZE", 0,
