LWFS_BOOSTER_FSTAB = "booster.fstab"
LWFS_UNFS3_EXPORTS = "boosterexports"
LWFS_CIFS_CONFIG = "boostersmb.conf"
LOGDIR = "/var/log/lwfs"
fstype = "lwfs"
...
class CreateBooster:
    ...
    def configure_booster_fstab (self):
        ...
        if self.conf_dir:
            booster_fstab_fd = file ("%s/%s" % (str(self.conf_dir),
                                                LWFS_BOOSTER_FSTAB), "a")
        else:
            booster_fstab_fd = file (LWFS_BOOSTER_FSTAB, "a")
    ...
    def configure_nfs_booster (self):
        ...
        if self.conf_dir:
            nfs_exports_fd = file ("%s/%s" % (str(self.conf_dir),
                                              LWFS_UNFS3_EXPORTS), "a")
        else:
            nfs_exports_fd = file (LWFS_UNFS3_EXPORTS, "a")
    ...
    def configure_cifs_booster (self):
        ...
        if self.conf_dir:
            ...
            cifs_config_fd = file (LWFS_CIFS_CONFIG, "a")
        cifs_config_fd.write ("[%s]\n" % self.volume_name)
        cifs_config_fd.write ("comment = %s volume served by LW\n" %
                              self.volume_name)
