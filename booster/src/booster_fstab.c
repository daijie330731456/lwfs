...
lwfs_fstab_t *
lwfs_fstab_init (const char *file, const char *mode)
{
        lwfs_fstab_t *handle = NULL;
        handle = calloc (1, sizeof (lwfs_fstab_t));
        if (!handle) {
...
int
lwfs_fstab_close (lwfs_fstab_t *h)
{
        if (!h)
                return -1;
...
/* Read one mount table entry from STREAM.  Returns a pointer to storage
   ...
   check).  */
struct lwfs_mntent *
__lwfs_fstab_getent (FILE *stream, struct lwfs_mntent *mp,
...
struct lwfs_mntent *
lwfs_fstab_getent (lwfs_fstab_t *h)
{
        if (!h)
                return NULL;
        ...
        return __lwfs_fstab_getent (h->fp, &h->tmpent, h->buf,
                                         GF_MNTENT_BUFSIZE);
}
...
int
lwfs_fstab_addent (lwfs_fstab_t *h,
                const struct lwfs_mntent *mnt)
{
        struct lwfs_mntent mntcopy = *mnt;
        if (!h)
                return -1;
...
char *
lwfs_fstab_hasoption (const struct lwfs_mntent *mnt, const char *opt)
{
...
void
clean_init_params (lwfs_init_params_t *ipars)
{
        if (!ipars)
                return;
...
void
booster_mount (struct lwfs_mntent *ent)
{
        char                    *opt = NULL;
        lwfs_init_params_t ipars;
        ...
        if ((strcmp (ent->mnt_type, "lwfs") != 0)) {
                gf_log ("booster-fstab", GF_LOG_ERROR, "Type is not lwfs");
                return;
        }

        memset (&ipars, 0, sizeof (lwfs_init_params_t));
        if (ent->mnt_fsname)
                ipars.specfile = strdup (ent->mnt_fsname);

        opt = lwfs_fstab_hasoption (ent, "subvolume");
        if (opt)
                ipars.volume_name = get_option_value (opt);

        opt = lwfs_fstab_hasoption (ent, "log-file");
        if (!opt)
                opt = lwfs_fstab_hasoption (ent, "logfile");

        if (opt)
                ipars.logfile = get_option_value (opt);

        opt = lwfs_fstab_hasoption (ent, "log-level");
        if (!opt)
                opt = lwfs_fstab_hasoption (ent, "loglevel");

        if (opt)
                ipars.loglevel = get_option_value (opt);

        /* Attribute cache timeout */
        opt = lwfs_fstab_hasoption (ent, "attr_timeout");
        if (opt) {
        ...
        if ((lwfs_mount (ent->mnt_dir, &ipars)) == -1)
...
booster_configure (char *confpath)
{
        int                     ret = -1;
        lwfs_fstab_t       *handle = NULL;
        struct lwfs_mntent *ent = NULL;

        if (!confpath)
                goto out;

        handle = lwfs_fstab_init (confpath, "r");
        if (!handle)
                goto out;

        while ((ent = lwfs_fstab_getent (handle)) != NULL)
                booster_mount (ent);

        lwfs_fstab_close (handle);
        ret = 0;
out:
        return ret;
}
