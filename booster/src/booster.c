...
#define BOOSTER_CONF_ENV_VAR    "LWFS_BOOSTER_FSTAB"
...
#define BOOSTER_LOG_ENV_VAR     "LWFS_BOOSTER_LOG"
...
do_open (int fd, const char *pathname, int flags, mode_t mode, booster_op_t op)
{
        ...
        size = fgetxattr (fd, "user.lwfs-booster-volfile", NULL, 0);
        if (size == -1) {
                gf_log ("booster", GF_LOG_ERROR, "Xattr "
                        "user.lwfs-booster-volfile not found: %s",
                        strerror (errno));
                goto out;
        }
        ...
        ret = fgetxattr (fd, "user.lwfs-booster-volfile", specfile,
                         size);
        if (ret == -1) {
                gf_log ("booster", GF_LOG_ERROR, "Xattr "
                        "user.lwfs-booster-volfile not found: %s",
        ...
        size = fgetxattr (fd, "user.lwfs-booster-mount", NULL, 0);
        if (size == -1) {
                gf_log ("booster", GF_LOG_ERROR, "Xattr "
                        "user.lwfs-booster-mount not found: %s",
                        strerror (errno));
                goto out;
        }
        ...
        ret = fgetxattr (fd, "user.lwfs-booster-mount", mount_point, size);
        if (ret == -1) {
                gf_log ("booster", GF_LOG_ERROR, "Xattr "
                        "user.lwfs-booster-mount not found: %s",
                        strerror (errno));
        ...
        ret = lwfs_mount (mount_point, &iparams);
        if (ret == -1) {
                if (errno != EEXIST) {
        ...
        case BOOSTER_OPEN:
                ...
                fh = lwfs_open (pathname, flags, mode);
                break;

        case BOOSTER_CREAT:
                gf_log ("booster", GF_LOG_TRACE, "Booster create call");
                fh = lwfs_creat (pathname, mode);
                break;
        }
...
booster_open (const char *pathname, int use64, int flags, ...)
{
         ...
         * 1. We depend on liblwfsclient library to perform
...
pread (int fd, void *buf, size_t count, unsigned long offset)
{
        ...
        if (!glfs_fd) { 
                ...
                ret = lwfs_pread (glfs_fd, buf, count, offset);
...
read (int fd, void *buf, size_t count)
{
        ...
        if (!glfs_fd) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_read (glfs_fd, buf, count);
                booster_fdptr_put (glfs_fd);
        }
...
readv (int fd, const struct iovec *vector, int count)
{
        ...
        if (!glfs_fd) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
		ret = lwfs_readv (glfs_fd, vector, count);
                booster_fdptr_put (glfs_fd);
        }
...
write (int fd, const void *buf, size_t count)
{
        ...
        if (!glfs_fd) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_write (glfs_fd, buf, count);
                booster_fdptr_put (glfs_fd);
        }
...
writev (int fd, const struct iovec *vector, int count)
{
        ...
        if (!glfs_fd) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_writev (glfs_fd, vector, count);
                booster_fdptr_put (glfs_fd);
        }
...
pwrite (int fd, const void *buf, size_t count, unsigned long offset)
{
        ...
        if (!glfs_fd) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_pwrite (glfs_fd, buf, count, offset);
                booster_fdptr_put (glfs_fd);
        }
...
lseek (int filedes, unsigned long offset, int whence)
{
        ...
        if (glfs_fd) {
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_lseek (glfs_fd, offset, whence);
                booster_fdptr_put (glfs_fd);
        } else {
                gf_log ("booster", GF_LOG_TRACE, "Not a booster fd");
                if (real_lseek == NULL) {
...
{
        int ret = -1;
        lwfs_file_t old_glfs_fd = NULL, new_glfs_fd = NULL;
...
chown (const char *pathname, uid_t owner, gid_t group)
{
        ...
        ret = lwfs_chown (pathname, owner, group);
...
fchown (int fd, uid_t owner, gid_t group)
{
        ...
        if (!fh) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_fchown (fh, owner, group);
...
booster_init (void)
{
        ...
        real_close (pipefd[1]);
        /* liblwfsclient based VMPs should be inited only
...
booster_cleanup (void)
{
         ...
         * liblwfsclient.
         * We should be satisfied with having cleaned up lwfs contexts.
         */
        lwfs_umount_all ();
	lwfs_reset ();
}
...
symlink (const char *oldpath, const char *newpath)
{
        ...
        gf_log ("booster", GF_LOG_TRACE, "symlink: old: %s, new: %s",
                oldpath, newpath);
        ret = lwfs_symlink (oldpath, newpath);
        if ((ret == -1) && (errno != ENODEV)) {
...
readlink (const char *path, char *buf, size_t bufsize)
{
        ...
        ret = lwfs_readlink (path, buf, bufsize);
        if ((ret == -1) && (errno != ENODEV)) {
...
realpath (const char *path, char *resolved_path)
{
        ...
        res = lwfs_realpath (path, resolved_path);
...
opendir (const char *path)
{
        ...
        gdir = lwfs_opendir (path);
        if (gdir) {
                gf_log ("booster", GF_LOG_TRACE, "LW dir opened");
...
booster_false_readdir_r (DIR *dir, struct dirent *entry, struct dirent **result)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "readdir_r on lw");
                ret = lwfs_readdir_r ((lwfs_dir_t)bh->dirh, entry,
                                           result);
                           ...
                           struct dirent64 **result)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "readdir_r on lw");
                ret = lwfs_readdir_r ((lwfs_dir_t)bh->dirh,
                                           (struct dirent *)entry,
                                           (struct dirent **)result);
        } else if (bh->type == BOOSTER_POSIX_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "readdir_r on posix");
...
booster_false_readdir (DIR *dir)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "readdir on lw");
                dirp = lwfs_readdir ((lwfs_dir_t)bh->dirh);
        } else if (bh->type == BOOSTER_POSIX_DIR) {
...
closedir (DIR *dh)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "closedir on lw");
                ret = lwfs_closedir ((lwfs_dir_t)bh->dirh);
        } else if (bh->type == BOOSTER_POSIX_DIR) {
...
booster_xstat64 (int ver, const char *path, void *buf)
{
        ...
        ret = lwfs_stat (path, (struct stat *)sbuf);
        if ((ret == -1) && (errno != ENODEV)) {
...
booster_fxstat64 (int ver, int fd, void *buf)
{
        ...
        if (!fh) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_fstat (fh, (struct stat *)sbuf);
                booster_fdptr_put (fh);
        }
...
booster_lxstat64 (int ver, const char *path, void *buf)
{
        ...
        ret = lwfs_lstat (path, (struct stat *)sbuf);
        if ((ret == -1) && (errno != ENODEV)) {
...
booster_statfs64 (const char *pathname, struct statfs64 *buf)
{
        ...
        ret = lwfs_statfs (pathname, (struct statfs *)buf);
        if ((ret == -1) && (errno != ENODEV)) {
...
booster_statvfs64 (const char *pathname, struct statvfs64 *buf)
{
        ...
        ret = lwfs_statvfs (pathname, (struct statvfs *)buf);
        if ((ret == -1) && (errno != ENODEV)) {
...
getxattr (const char *path, const char *name, void *value, size_t size)
{
        ...
        gf_log ("booster", GF_LOG_TRACE, "getxattr: path %s, name %s", path,
                name);
        ret = lwfs_getxattr (path, name, value, size);
        if ((ret == -1) && (errno != ENODEV)) {
...
lgetxattr (const char *path, const char *name, void *value, size_t size)
{
        ...
        gf_log ("booster", GF_LOG_TRACE, "lgetxattr: path %s, name %s", path,
                name);
        ret = lwfs_lgetxattr (path, name, value, size);
        if ((ret == -1) && (errno != ENODEV)) {
...
lchown (const char *path, uid_t owner, gid_t group)
{
        ...
        ret = lwfs_lchown (path, owner, group);
        if ((ret == -1) && (errno != ENODEV)) {
...
booster_rewinddir (DIR *dir)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "rewinddir on lwfs");
                lwfs_rewinddir ((lwfs_dir_t)bh->dirh);
        } else if (bh->type == BOOSTER_POSIX_DIR) {
...
booster_seekdir (DIR *dir, off_t offset)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "seekdir on lwfs");
                lwfs_seekdir ((lwfs_dir_t)bh->dirh, offset);
         } else if (bh->type == BOOSTER_POSIX_DIR) {
...
booster_telldir (DIR *dir)
{
        ...
        if (bh->type == BOOSTER_GL_DIR) {
                gf_log ("booster", GF_LOG_TRACE, "telldir on lwfs");
                offset = lwfs_telldir ((lwfs_dir_t)bh->dirh);
        } else if (bh->type == BOOSTER_POSIX_DIR) {
...
sendfile (int out_fd, int in_fd, off_t *offset, size_t count)
{
        ...
        gf_log ("booster", GF_LOG_TRACE, "sendfile: in fd %d, out fd %d, offset"
                ...
                count);
        /*
         * handle sendfile in booster only if in_fd corresponds to a lwfs
         * file handle 
         */
        ...
        if (!in_fh) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_sendfile (out_fd, in_fh, offset, count);
...
fcntl (int fd, int cmd, ...)
{
	...
	case F_DUPFD:
                 ...
                 *        etc flags in liblwfsclient. Commenting it out for
	...
	case F_NOTIFY:
                ...
                if (glfs_fd) {
                        gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                        ret = lwfs_fcntl (glfs_fd, cmd, arg);
                } else {
                        if (!real_fcntl) {
                                errno = ENOSYS;
                                goto out;
	...
	case F_SETLKW:
        ...
        case F_SETLKW64:
                ...
                if (glfs_fd) {
                        gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                        ret = lwfs_fcntl (glfs_fd, cmd, lock);
                } else {
                        if (!real_fcntl) {
                                errno = ENOSYS;
                                goto out;
          ...
          int flags)
{
        ...
        ret = lwfs_setxattr (path, name, value, size, flags);
        if ((ret == -1) && (errno != ENODEV)) {
           ...
           int flags)
{
        ...
        ret = lwfs_lsetxattr (path, name, value, size, flags);
        if ((ret == -1) && (errno != ENODEV)) {
...
fsetxattr (int fd, const char *name, const void *value, size_t size, int flags)
{
        ...
        if (!fh) {
                ...
                gf_log ("booster", GF_LOG_TRACE, "Is a booster fd");
                ret = lwfs_fsetxattr (fh, name, value, size, flags);
                booster_fdptr_put (fh);
        }
...
booster_lib_init (void)
{
	...
	RESOLVE (telldir);
         ...
         * above so that the socket based IO calls in liblwfsclient
