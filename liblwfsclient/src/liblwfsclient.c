...
#include "transport.h"
#include "event.h"
#include "liblwfsclient.h"
#include "liblwfsclient-internals.h"
#include "compat.h"
...
#define LIBGF_XL_NAME "liblwfsclient"
#define LIBLWFS_INODE_TABLE_LRU_LIMIT 1000 //14057
#define LIBGF_SENDFILE_BLOCK_SIZE 4096
#define LIBGF_READDIR_BLOCK     4096
#define libgf_path_absolute(path) ((path)[0] == '/')

static inline xlator_t *
liblwfs_graph (xlator_t *graph);
int32_t libgf_client_readlink (liblwfs_client_ctx_t *ctx, loc_t *loc,
                                        char *buf, size_t bufsize);

int
libgf_realpath_loc_fill (liblwfs_client_ctx_t *ctx, char *link,
                         loc_t *targetloc);
...
/* Ensures only one thread is ever calling lwfs_mount.
...
{
        uint64_t ptr = 0;
	liblwfs_client_inode_ctx_t *ctx = NULL;
	
	inode_ctx_del (inode, this, &ptr);
        ctx = (liblwfs_client_inode_ctx_t *)(long) ptr;
...
liblwfs_client_fd_ctx_t *
libgf_get_fd_ctx (fd_t *fd)
{
        uint64_t                        ctxaddr = 0;
        liblwfs_client_fd_ctx_t    *ctx = NULL;
        ...
        ctx = (liblwfs_client_fd_ctx_t *)(long)ctxaddr;
...
liblwfs_client_fd_ctx_t *
libgf_alloc_fd_ctx (liblwfs_client_ctx_t *ctx, fd_t *fd, char *vpath)
{
        liblwfs_client_fd_ctx_t    *fdctx = NULL;
        uint64_t                        ctxaddr = 0;
...
{
        liblwfs_client_fd_ctx_t    *fd_ctx = NULL;
 ...
 */
int
libgf_dcache_update (liblwfs_client_ctx_t *ctx, fd_t *fd,
                     gf_dirent_t *entries)
{
        liblwfs_client_fd_ctx_t    *fd_ctx = NULL;
        int                             op_ret = -1;
...
int
libgf_dcache_readdir (liblwfs_client_ctx_t *ctx, fd_t *fd,
...
liblwfs_client_inode_ctx_t *
libgf_alloc_inode_ctx (liblwfs_client_ctx_t *ctx, inode_t *inode)
{
        uint64_t                                ctxaddr = 0;
        liblwfs_client_inode_ctx_t         *ictx = NULL;
...
int
libgf_transform_iattr (liblwfs_client_ctx_t *libctx, inode_t *inode,
                       struct stat *buf)
{
...
{
        liblwfs_client_inode_ctx_t *inode_ctx = NULL;
        time_t                          current = 0;
        int                             op_ret = -1;
...
int
libgf_is_iattr_cache_valid (liblwfs_client_ctx_t *ctx, inode_t *inode,
...
static call_frame_t *
get_call_frame_for_req (liblwfs_client_ctx_t *ctx, char d)
{
...
libgf_client_notify (xlator_t *this, 
                     ...
                     ...)
{
        liblwfs_client_private_t *priv = this->private;
...
lwfs_handle_t 
lwfs_init (lwfs_init_params_t *init_ctx, uint32_t fakefsid)
{
        liblwfs_client_ctx_t *ctx = NULL;
        liblwfs_client_private_t *priv = NULL;
        FILE *specfp = NULL;
	...
	uint32_t xl_count = 0;
        ...
        if (!ctx) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: out of memory\n",
	...
	/* FIXME: why is count hardcoded to 16384 */
        ...
        if (init_ctx->loglevel) {
			...
			fprintf (stderr, 
				 "liblwfsclient: %s:%s():%d: Unrecognized log-level \"%s\", possible values are \"DEBUG|WARNING|[ERROR]|CRITICAL|NONE|TRACE\"\n",
                                 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                                 init_ctx->loglevel);
	...
	if (first_init)
        {
                ...
                if (getcwd (buf, PATH_MAX) == NULL) {
                        fprintf (stderr, "liblwfsclient: cannot get "
			...
			FREE (ctx->gf_ctx.cmd_args.log_file);
                ...
                if ((buf[len - 1] != '/')) {
                        if ((len + 2) > PATH_MAX) {
                                errno = ENAMETOOLONG;
                                fprintf (stderr, "liblwfsclient: cannot"
                ...
                if (ret == -1) {
			fprintf (stderr, 
				 "liblwfsclient: %s:%s():%d: failed to open logfile \"%s\"\n", 
				 __FILE__, __PRETTY_FUNCTION__, __LINE__, 
        ...
        if (init_ctx->specfp) {
			...
			fprintf (stderr, 
				 "liblwfsclient: %s:%s():%d: fseek on volume file stream failed (%s)\n",
                                 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                                 strerror (errno));
			FREE (ctx->gf_ctx.cmd_args.log_file);
                        FREE (ctx->gf_ctx.pool);
        ...
        if (!specfp) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: could not open volfile: %s\n", 
			 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                         strerror (errno));
		FREE (ctx->gf_ctx.cmd_args.log_file);
	...
	graph = file_to_xlator_tree (&ctx->gf_ctx, specfp);
        if (!graph) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: cannot create configuration graph (%s)\n",
			 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                         strerror (errno));
        ...
        ctx->gf_ctx.graph = liblwfs_graph (graph);
        if (!ctx->gf_ctx.graph) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: graph creation failed (%s)\n",
			 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                         strerror (errno));
	...
	ctx->gf_ctx.xl_count = xl_count + 1;
        ...
        if (!priv) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: cannot allocate memory (%s)\n",
			 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                         strerror (errno));
        ...
        graph->private = priv;
        ctx->itable = inode_table_new (LIBLWFS_INODE_TABLE_LRU_LIMIT,
                                       graph);
        if (!ctx->itable) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: cannot create inode table\n",
			 __FILE__, __PRETTY_FUNCTION__, __LINE__);
		xlator_tree_free (graph); 
		FREE (ctx->gf_ctx.cmd_args.log_file);
                FREE (ctx->gf_ctx.cmd_args.volume_file);
	...
	ctx->gf_ctx.process_uuid = zr_build_process_uuid ();
		...
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: graph initialization failed\n",
			 __FILE__, __PRETTY_FUNCTION__, __LINE__);
		xlator_tree_free (graph);
		FREE (ctx->gf_ctx.cmd_args.log_file);
                FREE (ctx->gf_ctx.cmd_args.volume_file);
	...
	graph->notify (graph, GF_EVENT_PARENT_UP, graph);
		...
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: timer init failed (%s)\n", 
			 __FILE__, __PRETTY_FUNCTION__, __LINE__,
                         strerror (errno));
        ...
        if ((ret = pthread_create (&ctx->reply_thread, NULL, poll_proc,
                                   (void *)&ctx->gf_ctx))) {
		fprintf (stderr, 
			 "liblwfsclient: %s:%s():%d: reply thread creation failed\n", 
			 __FILE__, __PRETTY_FUNCTION__, __LINE__);
		xlator_graph_fini (graph);
		xlator_tree_free (graph);
		FREE (ctx->gf_ctx.cmd_args.log_file);
        ...
        if (ret == -1) {
                gf_log ("liblwfsclient", GF_LOG_ERROR, "lookup of /"
                        " failed");
                return NULL;
        }
...
struct vmp_entry *
libgf_init_vmpentry (char *vmp, lwfs_handle_t *vmphandle)
{
...
/* copies vmp from the vmp-entry having lwfs handle @handle, into @vmp */
char *
libgf_vmp_search_vmp (lwfs_handle_t handle, char *vmp, size_t vmp_size)
{
        char             *res   = NULL;
        struct vmp_entry *entry = NULL;
...
int
libgf_vmp_map_ghandle (char *vmp, lwfs_handle_t *vmphandle)
{
        int                     ret = -1;
...
int
lwfs_mount (char *vmp, lwfs_init_params_t *ipars)
{
        lwfs_handle_t      vmphandle = NULL;
        int                     ret = -1;
        ...
        pthread_mutex_lock (&mountlock);
        {
                ...
                vmphandle = lwfs_init (ipars, vmphash);
                if (!vmphandle) {
                        errno = EINVAL;
                        gf_log (LIBGF_XL_NAME, GF_LOG_ERROR, "LWFS context"
                                " init failed");
                        goto unlock;
                }
...
_libgf_umount (char *vmp)
{
        ...
        if (entry == NULL) {
                gf_log ("liblwfsclient", GF_LOG_ERROR,
        ...
        if (entry->handle == NULL) {
                gf_log ("liblwfsclient", GF_LOG_ERROR,
                        "path (%s) has no corresponding lwfs handle",
                        vmp);
                goto out;
        }

/*        ret = lwfs_fini (entry->handle); */
...
void
libgf_wait_for_frames_unwind (liblwfs_client_ctx_t *ctx)
{
        call_pool_t     *pool = NULL;
        int             canreturn = 0;
...
int 
lwfs_fini (lwfs_handle_t handle)
{
	liblwfs_client_ctx_t *ctx = handle;
                         ...
                         struct stat *postparent)
{
        ...
        liblwfs_client_ctx_t *ctx = frame->root->state;
	dict_t *xattr_req = NULL;
		...
		inode_lookup (inode);
                ...
                if ((local->fop.lookup.is_revalidate == 0) 
                    && (op_errno == ENOENT)) {
                        gf_log ("liblwfsclient", GF_LOG_DEBUG,
			...
			int32_t ret = 0;
                        ...
                        if (local->fop.lookup.size > 0) {
                                xattr_req = dict_new ();
                                ret = dict_set (xattr_req, "lwfs.content",
...
int32_t
libgf_client_lookup (liblwfs_client_ctx_t *ctx,
                     loc_t *loc,
                     struct stat *stbuf,
                     dict_t **dict,
		     dict_t *xattr_req)
...
int 
lwfs_glh_get (lwfs_handle_t handle, const char *path, void *buf,
                        size_t size, struct stat *stbuf)
{
	...
	name = basename (pathname);
        ...
        if (size) { 
                xattr_req = dict_new ();
                op_ret = dict_set (xattr_req, "lwfs.content",
        ...
        if (!op_ret && stbuf && (stbuf->st_size <= size) && dict && buf) {
                ...
                mem_data = dict_get (dict, "lwfs.content");
                if (mem_data) {
...
int
lwfs_get (const char *path, void *buf, size_t size, struct stat *stbuf)
{
        int                     op_ret = -1;
        lwfs_handle_t      h      = NULL;
        char                    vpath[PATH_MAX];
        ...
        op_ret = lwfs_glh_get (h, vpath, buf, size, stbuf);
...
libgf_client_lookup_async_cbk (call_frame_t *frame,
                               ...
                               struct stat *postparent)
{
        liblwfs_client_async_local_t *local = frame->local;
        lwfs_get_cbk_t lookup_cbk = local->fop.lookup_cbk.cbk;
        liblwfs_client_ctx_t *ctx = frame->root->state;
	lwfs_iobuf_t *iobuf = NULL;
	dict_t *xattr_req = NULL;
        inode_t *parent = NULL;
...
int
lwfs_get_async (lwfs_handle_t handle, 
		     const char *path,
		     size_t size, 
		     lwfs_get_cbk_t cbk,
		     void *cbk_data)
{
        loc_t *loc = NULL;
        liblwfs_client_ctx_t *ctx = handle;
        liblwfs_client_async_local_t *local = NULL;
	int32_t op_ret = 0;
	dict_t *xattr_req = NULL;
	char *name = NULL, *pathname = NULL;
...
size_t 
libgf_client_getxattr (liblwfs_client_ctx_t *ctx, 
                       loc_t *loc,
                       const char *name,
                       void *value,
                       size_t size)
...
ssize_t
__lwfs_glh_getxattr (lwfs_handle_t handle, const char *path,
...
ssize_t
lwfs_glh_getxattr (lwfs_handle_t handle, const char *path,
                        const char *name, void *value, size_t size)
{
        return __lwfs_glh_getxattr (handle, path, name, value, size,
                                         LIBGF_DO_GETXATTR);
}

ssize_t
lwfs_glh_lgetxattr (lwfs_handle_t handle, const char *path,
                         const char *name, void *value, size_t size)
{
        return __lwfs_glh_getxattr (handle, path, name, value, size,
                                         LIBGF_DO_LGETXATTR);
...
ssize_t
lwfs_getxattr (const char *path, const char *name, void *value,
                        size_t size)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        ...
        op_ret = __lwfs_glh_getxattr (h, vpath, name, value, size,
                                           LIBGF_DO_GETXATTR);
...
ssize_t
lwfs_lgetxattr (const char *path, const char *name, void *value,
                     size_t size)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
...
int 
libgf_client_open (liblwfs_client_ctx_t *ctx, 
                   loc_t *loc, 
                   fd_t *fd, 
                   int flags)
{
...
int 
libgf_client_creat (liblwfs_client_ctx_t *ctx,
                    loc_t *loc,
                    fd_t *fd,
                    int flags,
                    mode_t mode)
...
int 
libgf_client_opendir (liblwfs_client_ctx_t *ctx,
                      loc_t *loc,
                      fd_t *fd)
{
        call_stub_t *stub = NULL;
...
lwfs_file_t 
lwfs_glh_open (lwfs_handle_t handle, const char *path, int flags,...)
{
        loc_t loc = {0, };
        long op_ret = -1;
        fd_t *fd = NULL;
...
lwfs_file_t
lwfs_open (const char *path, int flags, ...)
{
        va_list                 ap;
        lwfs_file_t        fh   = NULL;
        lwfs_handle_t      h    = NULL; 
        ...
        if (flags & O_CREAT) {
                ...
                mode = va_arg (ap, mode_t);
                va_end (ap);
                fh = lwfs_glh_open (h, vpath, flags, mode);
        } else
                fh = lwfs_glh_open (h, vpath, flags);
out:
        return fh;
}

lwfs_file_t 
lwfs_glh_creat (lwfs_handle_t handle, const char *path, mode_t mode)
{
        gf_log (LIBGF_XL_NAME, GF_LOG_DEBUG, "path %s", path);
	return lwfs_glh_open (handle, path,
			       (O_CREAT | O_WRONLY | O_TRUNC), mode);
}

lwfs_file_t
lwfs_creat (const char *path, mode_t mode)
{
        lwfs_file_t        fh = NULL;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        fh = lwfs_glh_creat (h, vpath, mode);
...
int 
libgf_client_flush (liblwfs_client_ctx_t *ctx, fd_t *fd)
{
        call_stub_t *stub;
        int32_t op_ret;
        libgf_client_local_t *local = NULL;
...
int
libgf_client_setxattr (liblwfs_client_ctx_t *ctx, 
                       loc_t *loc,
                       const char *name,
                       const void *value,
                       size_t size,
...
int 
__lwfs_glh_setxattr (lwfs_handle_t handle, const char *path,
...
int
lwfs_glh_setxattr (lwfs_handle_t handle, const char *path,
                        const char *name, const void *value, size_t size,
                        int flags)
{
        return __lwfs_glh_setxattr (handle, path, name, value, size, flags
                                         , LIBGF_DO_SETXATTR);
}

int
lwfs_glh_lsetxattr (lwfs_handle_t handle, const char *path,
                         const char *name, const void *value, size_t size,
                         int flags)
{
        return __lwfs_glh_setxattr (handle, path, name, value, size, flags
...
int
lwfs_setxattr (const char *path, const char *name, const void *value,
                        size_t size, int flags)
{
        ...
        op_ret = __lwfs_glh_setxattr (h, vpath, name, value, size, flags,
                                           LIBGF_DO_SETXATTR);
...
int
lwfs_lsetxattr (const char *path, const char *name, const void *value,
                     size_t size, int flags)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
...
int
libgf_client_fsetxattr (liblwfs_client_ctx_t *ctx, 
                        fd_t *fd,
                        const char *name,
                        const void *value,
                        size_t size,
...
size_t 
libgf_client_fgetxattr (liblwfs_client_ctx_t *ctx, 
                        fd_t *fd,
                        const char *name,
                        void *value,
                        size_t size)
...
ssize_t 
lwfs_listxattr (lwfs_handle_t handle,
                     const char *path, 
                     char *list,
                     size_t size)
{
...
ssize_t 
lwfs_llistxattr (lwfs_handle_t handle,
                      const char *path, 
                      char *list,
                      size_t size)
{
...
ssize_t 
lwfs_flistxattr (lwfs_file_t fd, 
                      char *list,
                      size_t size)
{
        return ENOSYS;
...
int 
lwfs_removexattr (lwfs_handle_t handle, 
                       const char *path, 
                       const char *name)
{
        return ENOSYS;
...
int 
lwfs_lremovexattr (lwfs_handle_t handle, 
                        const char *path, 
                        const char *name)
{
        return ENOSYS;
...
int 
lwfs_fremovexattr (lwfs_file_t fd, 
                        const char *name)
{
        return ENOSYS;
}
...
int
libgf_client_iobuf_read (liblwfs_client_ctx_t *ctx, fd_t *fd, void *buf,
...
int
libgf_client_read (liblwfs_client_ctx_t *ctx, fd_t *fd, void *buf,
                   size_t size, off_t offset)
{
        int32_t op_ret = -1;
        int32_t ret = 0;
...
ssize_t
lwfs_read (lwfs_file_t fd, void *buf, size_t nbytes)
{
        int32_t op_ret = -1;
        off_t offset = 0;
        liblwfs_client_ctx_t *ctx = NULL;
...
ssize_t
libgf_client_iobuf_readv (liblwfs_client_ctx_t *ctx, fd_t *fd,
...
ssize_t
libgf_client_readv (liblwfs_client_ctx_t *ctx, fd_t *fd,
...
ssize_t
lwfs_readv (lwfs_file_t fd, const struct iovec *vec, int count)
{
        int32_t op_ret = -1;
        off_t offset = 0;
        liblwfs_client_ctx_t *ctx = NULL;
...
int
libgf_client_iobuf_write (liblwfs_client_ctx_t *ctx, fd_t *fd, char *addr,
                          size_t size, off_t offset)
{
...
int
libgf_client_writev (liblwfs_client_ctx_t *ctx, 
                     fd_t *fd, 
                     struct iovec *vector, 
                     int count, 
                     off_t offset)
...
int 
libgf_client_readdir (liblwfs_client_ctx_t *ctx, fd_t *fd,
                      struct dirent *dirp, off_t *offset)
{  
        call_stub_t *stub = NULL;
        int op_ret = -1;
...
int
lwfs_readdir_r (lwfs_dir_t dirfd, struct dirent *entry,
...
int
lwfs_getdents (lwfs_file_t fd, struct dirent *dirp,
                    unsigned int count)
{
        int op_ret = -1;
        liblwfs_client_ctx_t *ctx = NULL;
...
static int32_t
liblwfs_readv_async_cbk (call_frame_t *frame,
                              void *cookie,
                              xlator_t *this,
                              int32_t op_ret,
                              int32_t op_errno,
                              ...
                              struct iobref *iobref)
{
        ...
        lwfs_readv_cbk_t readv_cbk = local->fop.readv_cbk.cbk;
...
int
lwfs_read_async (lwfs_file_t fd, 
                      size_t nbytes, 
                      off_t offset,
                      lwfs_readv_cbk_t readv_cbk,
                      void *cbk_data)
{
        liblwfs_client_ctx_t *ctx;
        fd_t *__fd = (fd_t *)fd;
...
static int32_t
liblwfs_writev_async_cbk (call_frame_t *frame,
                               void *cookie,
                               xlator_t *this,
                               int32_t op_ret,
                               int32_t op_errno,
...
int32_t
lwfs_write_async (lwfs_file_t fd, 
                       const void *buf, 
                       size_t nbytes, 
                       off_t offset,
                       lwfs_write_cbk_t write_cbk,
...
off_t
lwfs_lseek (lwfs_file_t fd, off_t offset, int whence)
{
        off_t __offset = 0;
	int32_t op_ret = -1;
        fd_t *__fd = (fd_t *)fd;
...
int32_t 
libgf_client_stat (liblwfs_client_ctx_t *ctx, 
                   loc_t *loc,
                   struct stat *stbuf)
{
        call_stub_t *stub = NULL;
                                ...
                                loc_t *targetloc)
{
        ...
        targetloc->path = lwfs_glh_realpath (ctx, link, NULL);
...
int
__lwfs_stat (lwfs_handle_t handle, const char *path,
                        struct stat *buf, int whichstat)
{
	...
	if (op_ret == -1) {
        ...
        real_loc = &loc;
        /* The stat fop in lwfs calls lstat. So we have to
...
int
lwfs_glh_stat (lwfs_handle_t handle, const char *path,
                        struct stat *buf)
{
        return __lwfs_stat (handle, path, buf, LIBGF_DO_STAT);
}

int
lwfs_stat (const char *path, struct stat *buf)
{
        lwfs_handle_t      h      = NULL;
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        ...
        op_ret = lwfs_glh_stat (h, vpath, buf);
...
int
lwfs_glh_lstat (lwfs_handle_t handle, const char *path, struct stat *buf)
{
        return __lwfs_stat (handle, path, buf, LIBGF_DO_LSTAT);
}

int
lwfs_lstat (const char *path, struct stat *buf)
{
        lwfs_handle_t      h      = NULL;
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        ...
        op_ret = lwfs_glh_lstat (h, vpath, buf);
out:
        return op_ret;
}
...
int32_t
libgf_client_fstat (liblwfs_client_ctx_t *ctx, 
                    fd_t *fd, 
                    struct stat *buf)
{
        call_stub_t *stub = NULL;
...
int32_t 
lwfs_fstat (lwfs_file_t fd, struct stat *buf) 
{
        liblwfs_client_ctx_t *ctx;
        fd_t *__fd = (fd_t *)fd;
        liblwfs_client_fd_ctx_t *fd_ctx = NULL;
...
static int32_t
libgf_client_mkdir (liblwfs_client_ctx_t *ctx,
		    loc_t *loc,
		    mode_t mode)
{
	int32_t op_ret = -1;
...
int32_t
lwfs_glh_mkdir (lwfs_handle_t handle, const char *path, mode_t mode)
{
	liblwfs_client_ctx_t *ctx = handle;
	loc_t loc = {0, };
...
int32_t
lwfs_mkdir (const char *path, mode_t mode)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_mkdir (h, vpath, mode);
out:
        return op_ret;
}
...
static int32_t
libgf_client_rmdir (liblwfs_client_ctx_t *ctx, loc_t *loc)
{
        int32_t op_ret = -1;
        call_stub_t *stub = NULL;
        libgf_client_local_t *local = NULL;
...
int32_t
lwfs_glh_rmdir (lwfs_handle_t handle, const char *path)
{
	liblwfs_client_ctx_t *ctx = handle;
	loc_t loc = {0, };
	char *pathname = NULL, *name = NULL;
...
int
libgf_client_setattr (liblwfs_client_ctx_t *ctx, loc_t * loc,
...
int
lwfs_glh_chmod (lwfs_handle_t handle, const char *path, mode_t mode)
{
        int                             op_ret = -1;
        liblwfs_client_ctx_t       *ctx = handle;
        loc_t                           loc = {0, };
...
int
lwfs_chmod (const char *path, mode_t mode)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_chmod (h, vpath, mode);
out:
        return op_ret;
}
...
int
__lwfs_chown (lwfs_handle_t handle, const char *path, uid_t owner,
...
int
lwfs_glh_chown (lwfs_handle_t handle, const char *path, uid_t owner,
                     gid_t group)
{
        return __lwfs_chown (handle, path, owner, group, LIBGF_DO_CHOWN);
}

int
lwfs_chown (const char *path, uid_t owner, gid_t group)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_chown (h, vpath, owner, group);
...
int
lwfs_glh_lchown (lwfs_handle_t handle, const char *path, uid_t owner,
                     gid_t group)
{
        return __lwfs_chown (handle, path, owner, group, LIBGF_DO_LCHOWN);
}

int
lwfs_lchown (const char *path, uid_t owner, gid_t group)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_lchown (h, vpath, owner, group);
out:
        return op_ret;
}

lwfs_dir_t
lwfs_glh_opendir (lwfs_handle_t handle, const char *path)
{
        int                             op_ret = -1;
        liblwfs_client_ctx_t       *ctx = handle;
        loc_t                           loc = {0, };
...
lwfs_opendir (const char *path)
{
        ...
        dir = lwfs_glh_opendir (h, vpath);
out:
        return dir;
}

int
lwfs_closedir (lwfs_dir_t dirfd)
{
        int                             op_ret = -1;
        liblwfs_client_fd_ctx_t    *fdctx = NULL;
...
int
libgf_client_fsetattr (liblwfs_client_ctx_t *ctx, fd_t *fd,
                       struct stat *stbuf, int32_t valid)
{
        int                     op_ret = -1;
        libgf_client_local_t    *local = NULL;
...
int
lwfs_fchmod (lwfs_file_t fd, mode_t mode)
{
        liblwfs_client_fd_ctx_t    *fdctx = NULL;
        int                             op_ret = -1;
        struct stat                     stbuf = {0,};
...
int
lwfs_fchown (lwfs_file_t fd, uid_t uid, gid_t gid)
{
        int                             op_ret = -1;
        liblwfs_client_fd_ctx_t    *fdctx = NULL;
        struct stat                     stbuf = {0,};
...
int
libgf_client_fsync (liblwfs_client_ctx_t *ctx, fd_t *fd)
{
        libgf_client_local_t    *local = NULL;
        call_stub_t             *stub = NULL;
        int                     op_ret = -1;
...
int
libgf_client_ftruncate (liblwfs_client_ctx_t *ctx, fd_t *fd,
                                off_t length)
{
        libgf_client_local_t            *local = NULL;
        call_stub_t                     *stub = NULL;
...
int
lwfs_ftruncate (lwfs_file_t fd, off_t length)
{
        liblwfs_client_fd_ctx_t    *fdctx = NULL;
        int                             op_ret = -1;
...
int
libgf_client_link (liblwfs_client_ctx_t *ctx, loc_t *old, loc_t *new)
{
        call_stub_t                     *stub = NULL;
        libgf_client_local_t            *local = NULL;
        int                             op_ret = -1;
...
int
lwfs_glh_link (lwfs_handle_t handle, const char *oldpath,
                        const char *newpath)
{
        liblwfs_client_ctx_t       *ctx = handle;
        int                             op_ret = -1;
...
int
lwfs_link (const char *oldpath, const char *newpath)
{
        int                     op_ret = -1;
        char                    oldvpath[PATH_MAX];
        char                    newvpath[PATH_MAX];
        lwfs_handle_t      oldh = NULL;
        lwfs_handle_t      newh = NULL;
        ...
        /* Cannot hard link across lwfs mounts. */
        if (newh != oldh) {
                errno = EXDEV;
                goto out;
        }

        op_ret = lwfs_glh_link (newh, oldvpath, newvpath);
out:
        return op_ret;
}
...
int32_t
libgf_client_statvfs (liblwfs_client_ctx_t *ctx, loc_t *loc,
                      struct statvfs *buf)
{
        call_stub_t             *stub = NULL;
        libgf_client_local_t    *local = NULL;
...
int
lwfs_glh_statfs (lwfs_handle_t handle, const char *path,
                        struct statfs *buf)
{
...
int
lwfs_statfs (const char *path, struct statfs *buf)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h      = NULL;
        ...
        op_ret = lwfs_glh_statfs (h, vpath, buf);
out:
        return op_ret;
}

int
lwfs_glh_statvfs (lwfs_handle_t handle, const char *path,
                                struct statvfs *buf)
{
        int32_t                         op_ret = -1;
        loc_t                           loc = {0, };
...
int
lwfs_statvfs (const char *path, struct statvfs *buf)
{
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h      = NULL;
        int                     op_ret = -1;
        ...
        op_ret = lwfs_glh_statvfs (h, vpath, buf);
out:
        return op_ret;
}
...
int32_t
libgf_client_rename (liblwfs_client_ctx_t *ctx, loc_t *oldloc,
                     loc_t *newloc)
{
        int                             op_ret = -1;
        libgf_client_local_t            *local = NULL;
...
int
lwfs_glh_rename (lwfs_handle_t handle, const char *oldpath,
                      const char *newpath)
{
        int32_t                         op_ret = -1;
...
int
lwfs_rename (const char *oldpath, const char *newpath)
{
        int                     op_ret = -1;
        char                    oldvpath[PATH_MAX];
        char                    newvpath[PATH_MAX];
        ...
        op_ret = lwfs_glh_rename (oldh, oldvpath, newvpath);
out:
        return op_ret;
}
...
int
lwfs_glh_utimes (lwfs_handle_t handle, const char *path,
...
int
lwfs_utimes (const char *path, const struct timeval times[2])
{
        char                    vpath[PATH_MAX];
        int                     op_ret = -1;
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_utimes (h, vpath, times);
out:
        return op_ret;
}

int
lwfs_glh_utime (lwfs_handle_t handle, const char *path,
...
int
lwfs_utime (const char *path, const struct utimbuf *buf)
{
        char                    vpath[PATH_MAX];
        int                     op_ret = -1;
        lwfs_handle_t      h      = NULL;
        ...
        op_ret = lwfs_glh_utime (h, vpath, buf);
out:
        return op_ret;
}
...
static int32_t
libgf_client_mknod (liblwfs_client_ctx_t *ctx, loc_t *loc, mode_t mode,
                    dev_t rdev)
{
        int32_t                 op_ret = -1;
        call_stub_t             *stub = NULL;
...
int
lwfs_glh_mknod(lwfs_handle_t handle, const char *path, mode_t mode,
                        dev_t dev)
{
        liblwfs_client_ctx_t       *ctx = handle;
        loc_t                           loc = {0, };
...
int
lwfs_mknod(const char *pathname, mode_t mode, dev_t dev)
{
        char                    vpath[PATH_MAX]; 
        int                     op_ret = -1;
        lwfs_handle_t      h      = NULL;
...
int
lwfs_glh_mkfifo (lwfs_handle_t handle, const char *path, mode_t mode)
{
...
int
lwfs_mkfifo (const char *path, mode_t mode)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_mkfifo (h, vpath, mode);
out:
        return op_ret;
}
...
int
libgf_client_unlink (liblwfs_client_ctx_t *ctx, loc_t *loc)
{
        int                             op_ret = -1;
        libgf_client_local_t            *local = NULL;
        call_stub_t                     *stub = NULL;
...
int
lwfs_glh_unlink (lwfs_handle_t handle, const char *path)
{
        int32_t                         op_ret = -1;
        loc_t                           loc = {0, };
        liblwfs_client_ctx_t       *ctx = handle;
...
lwfs_unlink (const char *path)
{
        ...
        op_ret = lwfs_glh_unlink (h, vpath);
...
int32_t
libgf_client_symlink (liblwfs_client_ctx_t *ctx, const char *linkpath,
                      loc_t *loc)
{
        int                     op_ret = -1;
        libgf_client_local_t    *local = NULL;
...
int
lwfs_glh_symlink (lwfs_handle_t handle, const char *oldpath,
                       const char *newpath)
{
        int32_t                         op_ret = -1;
        liblwfs_client_ctx_t       *ctx = handle;
...
int
lwfs_symlink (const char *oldpath, const char *newpath)
{
        char                    vpath[PATH_MAX];
        int                     op_ret = -1;
        lwfs_handle_t      h      = NULL;
        ...
        op_ret = lwfs_glh_symlink (h, oldpath, vpath);
out:
        return op_ret;
}
...
int32_t
libgf_client_readlink (liblwfs_client_ctx_t *ctx, loc_t *loc, char *buf,
                       size_t bufsize)
{
        int                             op_ret = -1;
        libgf_client_local_t            *local = NULL;
...
ssize_t
lwfs_glh_readlink (lwfs_handle_t handle, const char *path, char *buf,
                                size_t bufsize)
{
        int32_t                         op_ret = -1;
        loc_t                           loc = {0, };
...
ssize_t
lwfs_readlink (const char *path, char *buf, size_t bufsize)
{
        char                    vpath[PATH_MAX];
        int                     op_ret = -1;
        lwfs_handle_t      h      = NULL;
        ...
        op_ret = lwfs_glh_readlink (h, vpath, buf, bufsize);
out:
        return op_ret;
}

char *
lwfs_glh_realpath (lwfs_handle_t handle, const char *path,
                                char *resolved_path)
{
        char                            *buf = NULL;
        char                            *rpath = NULL;
        ...
        if (path[0] == '/') {
                ...
                /*
                   FIXME: can $CWD be a valid path on lwfs server? hence is
        ...
        for (start = end = (char *)path; *end; start = end) {
                ...
                if ((end - start == 1) && (start[0] == '.')) {
                        ...
                        *dest = '\0';
                        ...
                        ret = lwfs_glh_lstat (handle, rpath, &stbuf);
                        if (ret == -1) {
                                gf_log ("liblwfsclient", GF_LOG_ERROR,
                                        "lwfs_glh_stat returned -1 for"
                                        " path (%s):(%s)", rpath,
                                        strerror (errno));
                                goto err;
                        }
                        ...
                        if (S_ISLNK (stbuf.st_mode)) {
                                ...
                                ret = lwfs_glh_readlink (handle, rpath,
                                                              buf,
                                                              path_max - 1);
                                if (ret < 0) {
                                        gf_log ("liblwfsclient",
                                                GF_LOG_ERROR,
                                                "lwfs_readlink returned %d"
                                                " for path (%s):(%s)",
                                ...
                                rpath = lwfs_glh_realpath (handle, buf,
                                                                rpath);
                                FREE (buf);
                                if (rpath == NULL) {
                                        goto out;
...
char *
lwfs_realpath (const char *path, char *resolved_path)
{
        char                    *res   = NULL;
        char                     vpath[PATH_MAX];
        lwfs_handle_t       h     = NULL;
        ...
        res = lwfs_glh_realpath (h, vpath, resolved_path);
        if (!res)
                goto out;
...
int
lwfs_glh_remove (lwfs_handle_t handle, const char *path)
{
        loc_t                           loc = {0, };
        int                             op_ret = -1;
        liblwfs_client_ctx_t       *ctx = handle;
...
int
lwfs_remove(const char *pathname)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_remove (h, vpath);
out:
        return op_ret;
}

void
lwfs_rewinddir (lwfs_dir_t dirfd)
{
        liblwfs_client_fd_ctx_t    *fd_ctx = NULL;
...
void
lwfs_seekdir (lwfs_dir_t dirfd, off_t offset)
{
        liblwfs_client_fd_ctx_t    *fd_ctx = NULL;
...
libgf_client_sendfile_read_cbk (int op_ret, int op_errno,
                                lwfs_iobuf_t *buf, void *cbk_data)
{
...
ssize_t
lwfs_sendfile (int out_fd, lwfs_file_t in_fd, off_t *offset,
                    size_t count)
{
        ssize_t                           ret = -1;
        ...
        while (count != 0) {
                 ...
                 */
                ret = lwfs_read_async (in_fd, size, off,
...
int
libgf_client_lk (liblwfs_client_ctx_t *ctx, fd_t *fd, int cmd,
                 struct flock *lock)
{
        call_stub_t          *stub = NULL;
        int32_t               op_ret;
...
int
lwfs_fcntl (lwfs_file_t fd, int cmd, ...)
{
        int                           ret = -1;
...
lwfs_fchdir (lwfs_file_t fd)
{
        ...
        /* FIXME: there is a race-condition between lwfs_fchdir and
           lwfs_close. If two threads of application call lwfs_fchdir
           and lwfs_close on the same fd, there is a possibility of
           lwfs_fchdir accessing freed memory of fd_ctx.
        */
...
char *
lwfs_getcwd (char *buf, size_t size)
{
        char              *res      = NULL;
        size_t             len      = 0; 
        loc_t              loc      = {0, };
...
int32_t 
libgf_client_truncate (liblwfs_client_ctx_t *ctx, 
...
int
lwfs_glh_truncate (lwfs_handle_t handle, const char *path,
                        off_t length)
{
        int32_t op_ret = -1;
        loc_t loc = {0, };
...
int
lwfs_truncate (const char *path, off_t length)
{
        int                     op_ret = -1;
        char                    vpath[PATH_MAX];
        lwfs_handle_t      h = NULL;
        ...
        op_ret = lwfs_glh_truncate (h, vpath, length);
out:
        return op_ret;
}
