...
#define LIBLWFS_CLIENT_DENTRY_LOC_PREPARE(_new_loc, _loc, _parent, \
                                               _resolved) do {          \
                size_t pathlen = 0;                                     \
 ...
 */
static inode_t *
__libgf_client_path_to_parenti (liblwfs_client_ctx_t *ctx,
                                ...
                                char **reslv)
{
        ...
        GF_VALIDATE_OR_GOTO("liblwfsclient-dentry", resolved_till, out);
        pathdup = strdup (path);
        GF_VALIDATE_OR_GOTO("liblwfsclient-dentry", pathdup, out);
        ...
        if (!libgf_is_iattr_cache_valid (ctx, parent, NULL,
                ...
                if (ret == -1) {
                        gf_log ("liblwfsclient-dentry", GF_LOG_ERROR,
 ...
 */
static int32_t
__do_path_resolve (loc_t *loc, liblwfs_client_ctx_t *ctx,
                   char lookup_basename)
{
	...
	char           *file = NULL;   
        ...
        if (parent) {
                inode_ref (parent);
                gf_log ("liblwfsclient-dentry", GF_LOG_DEBUG,
	...
	directory = dirname (pathname);
        ...
        while (strcmp (resolved, directory) != 0) 
        {
                ...
                LIBLWFS_CLIENT_DENTRY_LOC_PREPARE (new_loc, loc, parent,
                                                        resolved);
