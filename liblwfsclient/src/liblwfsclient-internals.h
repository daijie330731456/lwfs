...
#ifndef __LIBLWFSCLIENT_INTERNALS_H
#define __LIBLWFSCLIENT_INTERNALS_H
...
typedef struct liblwfs_client_ctx {
        lwfs_ctx_t gf_ctx;
 ...
 * API in liblwfsclient only set the offset. The consequence is that when
...
typedef struct liblwfs_client_async_local {
        void *cbk_data;
        union {
                struct {
                        fd_t *fd;
                        lwfs_readv_cbk_t cbk;
                        char update_offset;
                }readv_cbk;
    
                struct {
                        fd_t *fd;
                        lwfs_write_cbk_t cbk;
                }write_cbk;
...
#define LIBGF_INSTALL_SIGNAL_HANDLERS(signal_handlers)                  \
        do {                                                            \
                ...
                /* LIBGF_CLIENT_SIGNAL (SIGTERM, lwfs_cleanup_and_exit); */ \
        } while (0)
