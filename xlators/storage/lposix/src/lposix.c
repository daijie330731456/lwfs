...
typedef struct {
  	...
  	struct stat *stbuf;
	loc_t       *loc;
} lposix_xattr_filler_t;

int
lposix_forget (xlator_t *this, inode_t *inode)
{
...
static void
_lposix_xattr_get_set (dict_t *xattr_req,
    		      char *key,
    		      data_t *data,
    		      void *xattrargs)
{
    	lposix_xattr_filler_t *filler = xattrargs;
    	char     *value      = NULL;
	...
	if (!strcmp (key, "lwfs.content")
    	...
    	} else if (!strcmp (key, LWFS_OPEN_FD_COUNT)) {
		loc = filler->loc;
...
static int 
lposix_scale_st_ino (struct lposix_private *priv, struct stat *buf)
{
        int   i        = 0;
        int   ret      = -1;
        ino_t temp_ino = 0;
...
int
lposix_lstat_with_gen (xlator_t *this, const char *path, struct stat *stbuf_p)
{
        struct lposix_private  *priv    = NULL;
        int                    ret     = 0;
        char                   gen_key[1024] = {0, };
        ...
        ret = lposix_scale_st_ino (priv, &stbuf);
        if (ret == -1) {
...
int
lposix_fstat_with_gen (xlator_t *this, int fd, struct stat *stbuf_p)
{
        struct lposix_private  *priv    = NULL;
        int                    ret     = 0;
        char                   gen_key[1024] = {0, };
...
dict_t *
lposix_lookup_xattr_fill (xlator_t *this, const char *real_path, loc_t *loc,
    			 dict_t *xattr_req, struct stat *buf)
{
    	dict_t     *xattr             = NULL;
    	lposix_xattr_filler_t filler   = {0, };
	...
	filler.loc       = loc;
    	...
    	dict_foreach (xattr_req, _lposix_xattr_get_set, &filler);
out:
    	return xattr;
}
...
setgid_override (xlator_t *this, char *real_path, gid_t *gid)
{
        ...
        if (!tmp_path) {
                op_ret = -ENOMEM;
                gf_log ("[storage/lposix]", GF_LOG_ERROR,
                        "Out of memory");
                goto out;
        }
        ...
        op_ret = lposix_lstat_with_gen (this, parent_path, &parent_stbuf);
...
int32_t
lposix_lookup (call_frame_t *frame, xlator_t *this,
              loc_t *loc, dict_t *xattr_req)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);

        priv = this->private;

        op_ret   = lposix_lstat_with_gen (this, real_path, &buf);
        op_errno = errno;
		...
		xattr = lposix_lookup_xattr_fill (this, real_path, loc,
						 xattr_req, &buf);
        }
...
parent:
        if (loc->parent) {
                ...
                op_ret = lposix_lstat_with_gen (this, parentpath, &postparent);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
...
int32_t
lposix_stat (call_frame_t *frame,
            xlator_t *this,
            loc_t *loc)
{
        struct stat           buf       = {0,};
...
static int
lposix_do_utimes (xlator_t *this,
                 const char *path,
                 struct stat *stbuf)
{
        int32_t ret = -1;
...
int
lposix_setattr (call_frame_t *frame, xlator_t *this,
               loc_t *loc, struct stat *stbuf, int32_t valid)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);
        ...
        op_ret = lposix_lstat_with_gen (this, real_path, &statpre);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & GF_SET_ATTR_MODE) {
                op_ret = lposix_do_chmod (this, real_path, stbuf);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & (GF_SET_ATTR_UID | GF_SET_ATTR_GID)){
                op_ret = lposix_do_chown (this, real_path, stbuf, valid);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & (GF_SET_ATTR_ATIME | GF_SET_ATTR_MTIME)) {
                op_ret = lposix_do_utimes (this, real_path, stbuf);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        op_ret = lposix_lstat_with_gen (this, real_path, &statpost);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "setattr (lstat) on %s failed: %s", real_path,
...
int32_t
lposix_do_fchown (xlator_t *this,
                 int fd,
                 struct stat *stbuf,
                 int32_t valid)
{
...
int32_t
lposix_do_fchmod (xlator_t *this,
...
static int
lposix_do_futimes (xlator_t *this,
                  int fd,
                  struct stat *stbuf)
{
        errno = ENOSYS;
...
int
lposix_fsetattr (call_frame_t *frame, xlator_t *this,
                fd_t *fd, struct stat *stbuf, int32_t valid)
{
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;

        op_ret = lposix_fstat_with_gen (this, pfd->fd, &statpre);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & GF_SET_ATTR_MODE) {
                op_ret = lposix_do_fchmod (this, pfd->fd, stbuf);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & (GF_SET_ATTR_UID | GF_SET_ATTR_GID)) {
                op_ret = lposix_do_fchown (this, pfd->fd, stbuf, valid);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        if (valid & (GF_SET_ATTR_ATIME | GF_SET_ATTR_MTIME)) {
                op_ret = lposix_do_futimes (this, pfd->fd, stbuf);
                if (op_ret == -1) {
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR,
        ...
        op_ret = lposix_fstat_with_gen (this, pfd->fd, &statpost);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "fsetattr (fstat) failed on fd=%p: %s", fd,
...
int32_t
lposix_opendir (call_frame_t *frame, xlator_t *this,
               loc_t *loc, fd_t *fd)
{
        char *            real_path = NULL;
        int32_t           op_ret    = -1;
...
int32_t
lposix_getdents (call_frame_t *frame, xlator_t *this,
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        while ((dirent = readdir (dir))) {
                ...
                /* This is to reduce the network traffic, in case only
                   directory is needed from lposix */
			 ...
			 ZR_PATH_MAX - strlen (tmp_real_path));
                ...
                ret = lposix_lstat_with_gen (this, tmp_real_path, &buf);
...
int32_t
lposix_releasedir (xlator_t *this,
		  fd_t *fd)
{
        int32_t           op_ret   = -1;
        int32_t           op_errno = 0;
...
int32_t
lposix_readlink (call_frame_t *frame, xlator_t *this,
                loc_t *loc, size_t size)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);
        ...
        lstat_ret = lposix_lstat_with_gen (this, real_path, &stbuf);
        if (lstat_ret == -1) {
                op_ret = -1;
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
...
int32_t
lposix_mknod (call_frame_t *frame, xlator_t *this,
             loc_t *loc, mode_t mode, dev_t dev)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);
        ...
        op_ret = lposix_lstat_with_gen (this, real_path, &stbuf);
        ...
        op_ret = lposix_lstat_with_gen (this, parentpath, &preparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
...
janitor_get_next_fd (xlator_t *this)
{
        ...
        pthread_mutex_lock (&priv->janitor_lock);
        {
                ...
                pfd = list_entry (priv->janitor_fds.next, struct lposix_fd,
                                  list);
...
static void *
lposix_janitor_thread_proc (void *data)
{
        xlator_t *            this = NULL;
        struct lposix_private *priv = NULL;
        struct lposix_fd *pfd;
...
static void
lposix_spawn_janitor_thread (xlator_t *this)
{
        struct lposix_private *priv = NULL;
        int ret = 0;
        ...
        LOCK (&priv->lock);
        {
                if (!priv->janitor_present) {
                        ret = pthread_create (&priv->janitor, NULL,
                                              lposix_janitor_thread_proc, this);
...
int32_t
lposix_mkdir (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_unlink (call_frame_t *frame, xlator_t *this,
              loc_t *loc)
{
        int32_t                  op_ret    = -1;
        int32_t                  op_errno  = 0;
...
#ifndef WEIWEI
                        fd = open (real_path, O_RDONLY|O_NONBLOCK);
#else
...
int32_t
lposix_rmdir (call_frame_t *frame, xlator_t *this,
             loc_t *loc)
{
        int32_t op_ret    = -1;
        int32_t op_errno  = 0;
...
int32_t
lposix_symlink (call_frame_t *frame, xlator_t *this,
...
int
lposix_rename (call_frame_t *frame, xlator_t *this,
              loc_t *oldloc, loc_t *newloc)
{
        ...
        SET_FS_ID (frame->root->uid, frame->root->gid);
	real_oldpath = make_real_path_from_frame(frame, this, oldloc->path);
	real_newpath = make_real_path_from_frame(frame, this, newloc->path);
        ...
        op_ret = lposix_lstat_with_gen (this, oldparentpath, &preoldparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "pre-operation lstat on parent of %s failed: %s",
        ...
        op_ret = lposix_lstat_with_gen (this, newparentpath, &prenewparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "pre-operation lstat on parent of %s failed: %s",
        ...
        op_ret = lposix_lstat_with_gen (this, real_newpath, &stbuf);
        if ((op_ret == -1) && (errno == ENOENT)){
                was_present = 0;
        }
        ...
        op_ret = lposix_lstat_with_gen (this, oldparentpath, &postoldparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "post-operation lstat on parent of %s failed: %s",
        ...
        op_ret = lposix_lstat_with_gen (this, newparentpath, &postnewparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
                        "post-operation lstat on parent of %s failed: %s",
...
int
lposix_link (call_frame_t *frame, xlator_t *this,
            loc_t *oldloc, loc_t *newloc)
{
        int32_t               op_ret       = -1;
        int32_t               op_errno     = 0;
	...
	real_newpath = make_real_path_from_frame(frame, this, newloc->path);
        ...
        newparentpath = dirname (newpathdup);
        op_ret = lposix_lstat_with_gen (this, newparentpath, &preparent);
        if (op_ret == -1) {
                op_errno = errno;
        ...
        op_ret = lposix_lstat_with_gen (this, newparentpath, &postparent);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR, "lstat failed: %s: %s",
                        newparentpath, strerror (op_errno));
...
int32_t
lposix_truncate (call_frame_t *frame,
                xlator_t *this,
                loc_t *loc,
                off_t offset)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);
        ...
        op_ret = lposix_lstat_with_gen (this, real_path, &prebuf);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
        ...
        op_ret = lposix_lstat_with_gen (this, real_path, &postbuf);
        if (op_ret == -1) {
                op_errno = errno;
...
int32_t
lposix_create (call_frame_t *frame, xlator_t *this,
              ...
              fd_t *fd)
{
	...
	real_path = make_real_path_from_frame(frame, this, loc->path);
...
#ifndef WEIWEI
        _fd = open (real_path, _flags|O_NONBLOCK, mode);
#else
        ...
        op_ret = lposix_fstat_with_gen (this, _fd, &stbuf);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
...
int32_t
lposix_open (call_frame_t *frame, xlator_t *this,
...
#ifndef WEIWEI
        _fd = open (real_path, flags|O_NONBLOCK, 0);
#else
...
#ifndef HAVE_SET_FSID
        if (flags & O_CREAT) {
		if (was_present != 1){		// WEIWEI_20110319
			op_ret = chown (real_path, frame->root->uid, gid);
			if (op_ret == -1) {
				op_errno = errno;
				gf_log (this->name, GF_LOG_ERROR,
...
int
lposix_readv (call_frame_t *frame, xlator_t *this,
...
#ifndef WEIWEI
__lposix_pwritev (int fd, struct iovec *vector, int count, off_t offset)
{
        int32_t         op_ret = 0;
        int             idx = 0;
        int             retval = 0;
        ...
        for (idx = 0; idx < count; idx++) {
                retval = pwrite (fd, vector[idx].iov_base, vector[idx].iov_len,
                                 internal_off);
                if (retval == -1) {
			gf_log ("lposix-pwrite", GF_LOG_ERROR, 
				"quota pwrite failed %d", errno);
                        op_ret = -errno;
                        goto err;
                }
                op_ret += retval;
...
int32_t
__lposix_writev (int fd, struct iovec *vector, int count, off_t startoff,
                int odirect)
{
        int32_t         op_ret = 0;
        int             idx = 0;
        ...
        if (!odirect)
                return __lposix_pwritev (fd, vector, count, startoff);
...
#ifdef WEIWEI
        alloc_buf = GF_MALLOC (1 * (max_buf_size + align), gf_lposix_mt_char);
#else
	alloc_buf = MALLOC (1 * (max_buf_size + align));
        ...
        for (idx = 0; idx < count; idx++) {
                ...
                if (retval == -1) {
			gf_log ("lposix-pwrite", GF_LOG_ERROR, 
					"quota pwrite failed %s(%d)",strerror(errno), errno);
                        op_ret = -errno;
                        goto err;
                }
...
int32_t
lposix_writev (call_frame_t *frame, xlator_t *this,
              ...
              struct iobref *iobref)
{
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        op_ret = lposix_fstat_with_gen (this, _fd, &preop);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_ERROR,
...
#else
	op_ret = __lposix_writev (_fd, vector, count, offset,
			 (pfd->flags & O_DIRECT));
	if (op_ret < 0) {
		op_errno = -op_ret;
		op_ret = -1;
        ...
        if (op_ret >= 0) {
                ...
                ret = lposix_fstat_with_gen (this, _fd, &postop);
                if (ret == -1) {
			op_ret = -1;
                        op_errno = errno;
                        gf_log (this->name, GF_LOG_ERROR, 
...
int32_t
lposix_statfs (call_frame_t *frame, xlator_t *this,
              loc_t *loc)
{
        char *                 real_path = NULL;
        int32_t                op_ret    = -1;
...
int32_t
lposix_flush (call_frame_t *frame, xlator_t *this,
             fd_t *fd)
{
        int32_t           op_ret   = -1;
        int32_t           op_errno = 0;
...
int32_t
lposix_fsync (call_frame_t *frame, xlator_t *this,
             fd_t *fd, int32_t datasync)
{
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        op_ret = lposix_fstat_with_gen (this, _fd, &postop);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_DEBUG,
...
static int gf_lposix_xattr_enotsup_log;
             ...
             data_pair_t *trav, int flags)
{
        ...
        if (ZR_FILE_CONTENT_REQUEST(trav->key)) {
                ...
                if (sys_ret < 0) {
                        if (errno == ENOTSUP) {
                                GF_LOG_OCCASIONALLY(gf_lposix_xattr_enotsup_log,
...
int32_t
lposix_setxattr (call_frame_t *frame, xlator_t *this,
                   ...
                   const char *name, char **contents)
{
        ...
        op_ret = lposix_lstat_with_gen (this, real_filepath, &stbuf);
        if (op_ret == -1) {
                op_ret = -errno;
...
/**
 * lposix_getxattr - this function returns a dictionary with all the
 *                  key:value pair present as xattr. used for
 *                  both 'listxattr' and 'getxattr'.
 */
int32_t
lposix_getxattr (call_frame_t *frame, xlator_t *this,
                loc_t *loc, const char *name)
{
	...
	if (loc->inode && S_ISREG (loc->inode->st_mode) && name &&
	    (strcmp (name, "trusted.lwfs.location") == 0)) {
                ret = dict_set_static_ptr (dict, 
                                           "trusted.lwfs.location", 
                                           priv->hostname);
                if (ret < 0) {
                        gf_log (this->name, GF_LOG_WARNING,
...
int32_t
lposix_fgetxattr (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_fsetxattr (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_removexattr (call_frame_t *frame, xlator_t *this,
                   loc_t *loc, const char *name)
{
        int32_t op_ret    = -1;
        int32_t op_errno  = 0;
...
int32_t
lposix_fsyncdir (call_frame_t *frame, xlator_t *this,
...
void
lposix_print_xattr (dict_t *this,
		   char *key,
		   data_t *value,
		   void *data)
{
	gf_log ("lposix", GF_LOG_DEBUG,
...
/**
 * xattrop - xattr operations - for internal use by LWFS
 * @optype: ADD_ARRAY:
 *            dict should contain:
                ...
                LOCK (&inode->lock);
                {
                        ...
                        default:
                                gf_log (this->name, GF_LOG_ERROR,
                                        ...
                                        "a bug report to lw-devel@nongnu.org",
                                        optype, path);
                                op_ret = -1;
                                op_errno = EINVAL;
                                goto unlock;
...
int
lposix_xattrop (call_frame_t *frame, xlator_t *this,
...
int
lposix_fxattrop (call_frame_t *frame, xlator_t *this,
...
int
lposix_access (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_ftruncate (call_frame_t *frame, xlator_t *this,
...
ensure_file_type (xlator_t *this, char *pathname, mode_t mode)
{
        ...
        ret = lposix_lstat_with_gen (this, pathname, &stbuf);
        if (ret == -1) {
                op_ret = -errno;
                gf_log (this->name, GF_LOG_ERROR,
...
int
lposix_setdents (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_fstat (call_frame_t *frame, xlator_t *this,
             fd_t *fd)
{
        int                   _fd      = -1;
        int32_t               op_ret   = -1;
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        op_ret = lposix_fstat_with_gen (this, _fd, &buf);
        if (op_ret == -1) {
                op_errno = errno;
...
static int gf_lposix_lk_log;

int32_t
lposix_lk (call_frame_t *frame, xlator_t *this,
          fd_t *fd, int32_t cmd, struct flock *lock)
{
	...
	GF_LOG_OCCASIONALLY (gf_lposix_lk_log, this->name, GF_LOG_ERROR,
...
int32_t
lposix_finodelk (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_entrylk (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_fentrylk (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_do_readdir (call_frame_t *frame, xlator_t *this,
	...
	pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        while (filled <= size) {
                ...
                if ((whichop == GF_FOP_READDIRP) || (priv->span_devices)) {
                        ...
                        op_ret = lposix_lstat_with_gen (this, entry_path, &stbuf);
                        if (-1 == op_ret)
                                continue;
                } else
...
int32_t
lposix_readdir (call_frame_t *frame, xlator_t *this,
               fd_t *fd, size_t size, off_t off)
{
        lposix_do_readdir (frame, this, fd, size, off, GF_FOP_READDIR);
        return 0;
}


int32_t
lposix_readdirp (call_frame_t *frame, xlator_t *this,
                fd_t *fd, size_t size, off_t off)
{
        lposix_do_readdir (frame, this, fd, size, off, GF_FOP_READDIRP);
        return 0;
}


int32_t
lposix_stats (call_frame_t *frame, xlator_t *this,
             int32_t flags)

{
        ...
        struct statvfs         buf     = {0,};
        struct timeval         tv      = {0,};
        struct lposix_private * priv = (struct lposix_private *)this->private;
...
int32_t
lposix_checksum (call_frame_t *frame, xlator_t *this,
...
int32_t
lposix_rchecksum (call_frame_t *frame, xlator_t *this,
...
#ifndef IOCTL /* wanghy add */
int32_t
lposix_ioctl (call_frame_t *frame, xlator_t *this,
              fd_t *fd, uint32_t cmd, uint64_t arg)
{
        ...
        struct lposix_fd *      pfd = NULL;
        int32_t                  ret = -1;
        struct lov_user_md *lum = (struct lov_user_md *)arg;

        VALIDATE_OR_GOTO (frame, out);
        VALIDATE_OR_GOTO (this, out);
        VALIDATE_OR_GOTO (fd, out);
        VALIDATE_OR_GOTO (cmd, out);
        VALIDATE_OR_GOTO (this->private, out);
        ...
        if (ret < 0) {
                op_errno = -ret;
                gf_log (this->name, GF_LOG_DEBUG,
                        "WANGHY: enter lposix_ioctl: pfd is NULL from fd=%p\n", fd);
                goto out;
        }
        pfd = (struct lposix_fd *)(long)tmp_pfd;
        ...
        switch(cmd){
                ...
                case IOC_MDC_GETFILESTRIPE:
                        gf_log (this->name, GF_LOG_DEBUG,
                                "WANGHY: enter lposix_ioctl: (case IOC_MDC_GETFILESTRIPE)arg=%s\n", arg);
                        int v1 = sizeof(struct lov_user_md_v1) +
                                 LOV_MAX_STRIPE_COUNT * sizeof(struct lov_user_ost_data_v1);
                        int v3 = sizeof(struct lov_user_md_v3) +
                                 LOV_MAX_STRIPE_COUNT * sizeof(struct lov_user_ost_data_v1);
                        int join = sizeof(struct lov_user_md_join) +
                                   LOV_MAX_STRIPE_COUNT * sizeof(struct lov_user_ost_data_join);
                        int v = v1 > v3 ? v1 : v3;
                        int lum_size= v > join ? v : join;
                        lum = malloc(lum_size);
                        memcpy((char *)lum, (char *)arg, lum_size);
                        break;
                default:
                        break;
        }

        op_ret = ioctl (_fd, cmd, (void *)lum);
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_DEBUG,
                        "WANGHY: enter lposix_ioctl: ioctl failed on _fd=%d: %s\n",
                        _fd, strerror (op_errno));
                goto out;
        }
 out:

        STACK_UNWIND_STRICT (ioctl, frame, op_ret, op_errno, cmd, lum);
        ...
        ...)
{
        switch (event)
                ...
                case GF_EVENT_PARENT_UP:
                        {
                                /* Tell the parent that lposix xlator is up */
...
init_gen_seq(void)
{
	...
	if ( fd < 0 ) {
		gf_log ("init_gen", GF_LOG_ERROR,
			"socket failed");
		goto out;
	}
	strcpy((char *)(buf.ifr_name),"eth0");
	ret = ioctl(fd, SIOCGIFADDR, (char *)&buf);
	if ( ret == 0 ) {
		paddr = (struct sockaddr_in *)(&buf.ifr_addr);
		serno = paddr->sin_addr.s_addr;
	} else {
		gf_log ("init_gen", GF_LOG_ERROR,
			"ioctl failed");
	}
...
out:
        time64 = time (NULL);
	time64 = (time64 & 0x00ffffff) | (serno & 0xff000000);
        serno = (time64 << 32);

	gf_log ("init_gen", GF_LOG_WARNING,
			"serno[%#lx]",serno);
	return serno;
}
#endif
...
init (xlator_t *this)
{
        ...
        int                    op_ret   = -1;
        struct stat            buf      = {0,};
        struct lposix_private * _private = NULL;
	...
	gf_boolean_t           tmp_bool = 0;
        ...
        if (this->children) {
                gf_log (this->name, GF_LOG_CRITICAL,
                        "FATAL: storage/lposix cannot have subvolumes");
                ret = -1;
                goto out;
        }
	...
	_private->gen_seq = init_gen_seq();
        ...
        lposix_spawn_janitor_thread (this);
...
fini (xlator_t *this)
{
        struct lposix_private *priv = this->private;
        sys_lremovexattr (priv->base_path, "trusted.lwfs.test");
        FREE (priv);
        return;
}
...
struct xlator_fops fops = {
        .lookup      = lposix_lookup,
        .stat        = lposix_stat,
        .opendir     = lposix_opendir,
        .readdir     = lposix_readdir,
        .readdirp    = lposix_readdirp,
        .readlink    = lposix_readlink,
        .mknod       = lposix_mknod,
        .mkdir       = lposix_mkdir,
        .unlink      = lposix_unlink,
        .rmdir       = lposix_rmdir,
        .symlink     = lposix_symlink,
        .rename      = lposix_rename,
        .link        = lposix_link,
        .truncate    = lposix_truncate,
        .create      = lposix_create,
        .open        = lposix_open,
        .readv       = lposix_readv,
        .writev      = lposix_writev,
        .statfs      = lposix_statfs,
        .flush       = lposix_flush,
        .fsync       = lposix_fsync,
        .setxattr    = lposix_setxattr,
        .fsetxattr   = lposix_fsetxattr,
        .getxattr    = lposix_getxattr,
        .fgetxattr   = lposix_fgetxattr,
        .removexattr = lposix_removexattr,
        .fsyncdir    = lposix_fsyncdir,
        .access      = lposix_access,
        .ftruncate   = lposix_ftruncate,
        .fstat       = lposix_fstat,
        .lk          = lposix_lk,
	.inodelk     = lposix_inodelk,
	.finodelk    = lposix_finodelk,
	.entrylk     = lposix_entrylk,
	.fentrylk    = lposix_fentrylk,
        .setdents    = lposix_setdents,
        .getdents    = lposix_getdents,
        .checksum    = lposix_checksum,
        .rchecksum   = lposix_rchecksum,
	.xattrop     = lposix_xattrop,
	.fxattrop    = lposix_fxattrop,
        .setattr     = lposix_setattr,
        .fsetattr    = lposix_fsetattr,
#ifndef IOCTL /* wanghy add */
        .ioctl       = lposix_ioctl,
#endif
};
...
struct xlator_cbks cbks = {
	.release     = lposix_release,
	.releasedir  = lposix_releasedir,
	.forget      = lposix_forget
};
