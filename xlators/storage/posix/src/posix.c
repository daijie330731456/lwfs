...
#ifndef JJH	/* lustre get generation cmd */
#define GET_GENERATION _IOR('f', 3, long)
#endif

#ifndef HXB
extern int quota_update(void *data);
#endif
...
#ifdef WEIWEI_20160516 
	/* use lustre get generation interface */
	int fd=0;
	fd = open(path,O_RDONLY|O_NONBLOCK);
	if ( fd < 0 ){ /* open error or not exist, just return */
		/* multiple daemon need enlarge lock */
		if (S_ISLNK(stbuf.st_mode) && (errno == ENOENT)) {
				/* ioctl fail then use seq */
...
gf_log (this->name, GF_LOG_ERROR,
		"open path %s failed, errno %d\n", path, errno);
		}
	} else { /* must exist */
		ret = ioctl(fd,GET_GENERATION,&gen_val_be); /* try ioctl */
		if ( ret < 0 ) { /* not support ioctl */
			/* ioctl fail then use seq */
			ret = sys_lgetxattr (path, gen_key, (void *) &gen_val_be,
				sizeof (gen_val_be));
        		if (ret == -1) {
...
#ifdef WEIWEI_20160518	/* use lustre get generation interface */
	ret = ioctl(fd,GET_GENERATION,&gen_val_be);
        if (ret < 0) {
...
#ifndef HXB
	SPACE_ENOUGH_OR_SETERR_GOTO (frame, this, op_errno, out);
#endif
        priv = this->private;
        VALIDATE_OR_GOTO (priv, out);
              ...
              fd_t *fd, uint32_t cmd, uint64_t arg)
{
        ...
        if (ret < 0) {
                op_errno = -ret;
                gf_log (this->name, GF_LOG_DEBUG,
                        "WANGHY: enter posix_ioctl: pfd is NULL from fd=%p\n", fd);
                goto out;
        }
        pfd = (struct posix_fd *)(long)tmp_pfd;
        ...
        switch(cmd){
                ...
                case IOC_MDC_GETFILESTRIPE:
                        gf_log (this->name, GF_LOG_DEBUG,
                                "WANGHY: enter posix_ioctl: (case IOC_MDC_GETFILESTRIPE)arg=%s\n", arg);
        ...
        if (op_ret == -1) {
                op_errno = errno;
                gf_log (this->name, GF_LOG_DEBUG,
                        "WANGHY: enter posix_ioctl: ioctl failed on _fd=%d: %s\n",
...
#ifndef WEIWEI_20121101
	gf_boolean_t quota_on = 1;
	tmp_data = dict_get (this->options, "quota-on");
        if (tmp_data) {
		if (gf_string2boolean (tmp_data->data,
				       &_private->quota_on) == -1) {
			ret = -1;
			gf_log (this->name, GF_LOG_ERROR,
				"wrong option provided for 'quota-on'");
			goto out;
		}
		if (_private->quota_on)
                        gf_log (this->name, GF_LOG_DEBUG,
                                "quota mode is enabled");
        }
#endif
...
#ifndef HXB
/* hexb@20100727
 * update quota using a special thread
 */
	if ( _private->quota_on ) {
		pthread_create(&_private->qthread,NULL,quota_update,_private->base_path);
		gf_log (this->name, GF_LOG_NORMAL, "start quota update thread success");
	}
#endif
 out:
        return ret;
