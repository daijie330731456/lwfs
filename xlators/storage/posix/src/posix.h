...
#ifndef HXB
/* hexb@2010727
 * quota check definition
 */
int is_quota_overflow(call_frame_t *, xlator_t *);

#define SPACE_ENOUGH_OR_SETERR_GOTO(frame, this, err, label) do { \
	struct posix_private *_priv = (struct posix_private *)this->private;	\
        if ( (_priv->quota_on) && (is_quota_overflow(frame,this)>0) ) {       \
                err = EDQUOT;                           \
