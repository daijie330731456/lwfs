                          ...
                          int child_index, dict_t *xattr)
{
	uint32_t        open_fd_count = 0;
        ...
        ret = dict_get_uint32 (xattr, LWFS_OPEN_FD_COUNT,
                               &open_fd_count);
        if (ret == 0)
                local->open_fd_count += open_fd_count;

        ret = dict_get_uint32 (xattr, LWFS_INODELK_COUNT,
                               &inodelk_count);
        if (ret == 0)
                local->inodelk_count += inodelk_count;

        ret = dict_get_uint32 (xattr, LWFS_ENTRYLK_COUNT,
                               &entrylk_count);
        if (ret == 0)
	    ...
	    loc_t *loc, dict_t *xattr_req)
{
	...
	ret = dict_set_uint64 (local->xattr_req, LWFS_OPEN_FD_COUNT, 0);
        ret = dict_set_uint64 (local->xattr_req, LWFS_INODELK_COUNT, 0);
        ret = dict_set_uint64 (local->xattr_req, LWFS_ENTRYLK_COUNT, 0);
...
static const char *no_lock_servers_warning_str = "You have set lock-server-count = 0. "
	...
	"are ABSOLUTELY SURE of what you are doing and WILL NOT HOLD LWFS "
