...
#ifndef HXB091211
#define GET_OP_NUM(op) ((op) & 0x0ffff)
#define GET_DEBUG_BIT(op) (((op) >> 16) & 0x01)
#endif
                  ...
                  int32_t op_ret, int32_t op_errno, uint32_t cmd, int64_t retaddr)
{
        ...
        switch(cmd){
                ...
                case IOC_MDC_GETFILESTRIPE:
                        ...
                        int v = v1 > v3 ? v1 : v3;
                        lum_size = v > join ? v : join;
                        break;
                default:
                        break;
        }

        hdrlen = gf_hdr_len (rsp, lum_size);
        hdr    = gf_hdr_new (rsp, lum_size);
        rsp    = gf_param (hdr);
        rsp->cmd = hton32(cmd);

        if(lum_size > 0)
                memcpy(rsp->lum, (struct lov_user_md *)retaddr, lum_size);
        ...
        switch(state->cmd){
                case LL_IOC_LOV_SETSTRIPE:
                        STACK_WIND (frame, server_ioctl_cbk,
                                    ...
                                    state->fd, state->cmd, &state->lum);
                        break;
                case LL_IOC_LOV_GETSTRIPE:
                case IOC_MDC_GETFILESTRIPE:
                        STACK_WIND (frame, server_ioctl_cbk,
                                    bound_xl, bound_xl->fops->ioctl,
                                    state->fd, state->cmd, state->resolve.path);
                        break;
                default:
                        break;
        }
...
err:
        server_ioctl_cbk (frame, NULL, frame->this, state->resolve.op_ret,
                          state->resolve.op_errno, state->cmd, NULL);
        return 0;
}
              ...
              struct iobuf *iobuf)
{
        ...
        switch(state->cmd){
                case LL_IOC_LOV_SETSTRIPE:
                        /* _IOC_NR(state->cmd==154 */
                        state->lum.lmm_magic = req->lum.lmm_magic;
                        state->lum.lmm_pattern = req->lum.lmm_pattern;
                        state->lum.lmm_stripe_size = req->lum.lmm_stripe_size;
                        state->lum.lmm_stripe_count = req->lum.lmm_stripe_count;
                        state->lum.lmm_stripe_offset = req->lum.lmm_stripe_offset;
                        strncpy(state->lum.lmm_pool_name, req->lum.lmm_pool_name, LOV_MAXPOOLNAME);
                        break;
                case LL_IOC_LOV_GETSTRIPE:
                case IOC_MDC_GETFILESTRIPE:
...
#ifndef HEXB
	/* @20100512 */
	if (strcmp(state->resolve.path, state->loc.path))
		gf_log ("server", GF_LOG_ERROR,
			"resolve path[%s] diff with old path[%s]",
			state->loc.path, state->resolve.path);
#endif
        STACK_WIND (frame, server_lookup_cbk,
                      ...
                      struct xlator_stats *stats)
{
        ...
        char                buffer[256] = {0,};
        int64_t             lwfsd_stats_nr_clients = 0;
        size_t              hdrlen = 0;
        size_t              buf_len = 0;
        int32_t             gf_errno = 0;
...
#ifndef HXB091211
        frame->root->op   = GET_OP_NUM(ntoh32 (hdr->op));
        frame->root->debug = GET_DEBUG_BIT(ntoh32 (hdr->op));
#else
