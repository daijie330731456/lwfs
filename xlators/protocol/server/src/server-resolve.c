...
resolve_entry_simple (call_frame_t *frame)
{
        ...
        if (!parent) {
                ...
                gf_log (this->name, GF_LOG_DEBUG, "%"PRId64": inode "
                		"parent is NULL for path (%s)",
              			resolve->par, resolve->path);

                inode = inode_grep (state->itable, parent, resolve->bname);
                if (inode != NULL) {
                        gf_log (this->name, GF_LOG_ERROR, "%"PRId64": inode "
        ...
        if (resolve->type == RESOLVE_NOT) {
                ...
                /*
                 * @20100612
                 * here unlink orphan inode from its parent, and forget
                 * lookups
                 */
                inode_unlink(inode, parent, resolve->bname);
                inode_unref (parent);
...
#ifndef JJH /* debug double cbk */
	LOCK(&frame->lock);
	if ( frame->complete ) {
                gf_log ("JJH", GF_LOG_ERROR,
                        "double reolve_done op[%d]\n",frame->root->op);
		UNLOCK(&frame->lock);
		return 0;
	} else {
		frame->complete = 1;
