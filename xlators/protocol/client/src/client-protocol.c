...
#ifndef HXB091211
#define SET_DEBUG_BIT(op) (int32_t)((op) | 0x010000)
#endif
...
#ifndef HXB091211
		if ( frame->root->debug )
			hdr->op   = hton32 (SET_DEBUG_BIT(op));
		else
			hdr->op   = hton32 (op);
#else
                hdr->op     = hton32 (op);
...
int
client_ioctl (call_frame_t *frame, xlator_t *this, fd_t *fd, uint32_t cmd, uint64_t arg)
{
        gf_hdr_common_t     *hdr = NULL;
        ...
        switch(cmd){
                case LL_IOC_LOV_SETSTRIPE:
                        ...
                        req->lum.lmm_magic=lum->lmm_magic;
                        req->lum.lmm_pattern=lum->lmm_pattern;
                        req->lum.lmm_stripe_size=lum->lmm_stripe_size;
                        req->lum.lmm_stripe_count=lum->lmm_stripe_count;
                        req->lum.lmm_stripe_offset=lum->lmm_stripe_offset;
                        strncpy(req->lum.lmm_pool_name, lum->lmm_pool_name, LOV_MAXPOOLNAME);
                        break;
		case LL_IOC_LOV_GETSTRIPE:
                case IOC_MDC_GETFILESTRIPE:
                        hdrlen = gf_hdr_len (req, STRLEN_0(path));
                        hdr    = gf_hdr_new (req, STRLEN_0(path));
                        GF_VALIDATE_OR_GOTO (this->name, hdr, unwind);
                        req    = gf_param (hdr);
                        req->fd = hton64 (remote_fd);
                        req->cmd   = cmd;
		...
		/* hexb@20100505 */
                case LWFS_IOC_SETOP:
                case LWFS_IOC_GETOP:
			STACK_UNWIND (frame, 0, 0, cmd, arg);
			if (hdr)
				free (hdr);
                        return 0;
                /* hexb@20100505 end */
                   ...
                   struct iobuf *iobuf)
{
        ...
        int32_t         op_errno = 0;
        int             cmd=-1;
        struct lum_user_md *lum = NULL;
        int v1,v3,join,lum_size;
        ...
        STACK_UNWIND (frame, op_ret, op_errno, cmd, rsp->lum);
        return 0;
}
#endif
