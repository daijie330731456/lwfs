/*
  Copyright (c) 2009-2010 LW, Inc. <http://www.lw.com>
               ...
               struct stat *buf, dict_t *dict, struct stat *postparent)
{
        ...
        content = dict_get (dict, LWFS_CONTENT_KEY);
...
qr_lookup (call_frame_t *frame, xlator_t *this, loc_t *loc, dict_t *xattr_req)
{
        ...
        if (!cached) {
                if (xattr_req) {
                        content = dict_get (xattr_req, LWFS_CONTENT_KEY);
                        if (content) {
                ...
                if ((conf->max_file_size > 0)
                    && (conf->max_file_size != requested_size)) {
                        ...
                        op_ret = dict_set (xattr_req, LWFS_CONTENT_KEY,
