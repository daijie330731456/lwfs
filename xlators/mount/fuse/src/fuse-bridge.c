...
#define LWFS_XATTR_LEN_MAX  65536

#ifndef JJH1101
#define MAX_FUSE_PROC_DELAY 2
#else
...
#ifndef HXB0315
static inode_t *
fuse_ino_to_inode (uint64_t gen, inode_table_t *table)
{
	int hash = 0;
        inode_t *inode = NULL;
	...
	if (gen == 1) {
		...
		inode = inode_get_by_gen(table, gen);
	}
        return inode;
}
static uint64_t
        ...
        if (name) {
			...
			if (!parent) {
				gf_log("fuse", GF_LOG_ERROR, 
					"parent %ld can not be found", par);
				goto fail;
			}
#endif
                        loc->parent = parent;
                ...
                inode_t *inode, struct stat *buf)
{
        ...
        if (op_ret == 0) {
                ...
                if (!buf->st_ino) {
                        gf_log ("lwfs-fuse", GF_LOG_WARNING,
...

{
	...
	if (!fuse_inode) {
		gf_log("fuse", GF_LOG_ERROR,
			"fuse inode NULL when forgetting");
		FREE(finh);
		return;
#endif
	}
...
gf_log ("lwfs-fuse", GF_LOG_TRACE, "fuse getattr entered\n");
        GET_STATE (this, finh, state);
...
gf_log ("lwfs-fuse", GF_LOG_TRACE, "fuse_opendir entered\n");
        GET_STATE (this, finh, state);
        ret = fuse_loc_fill (&state->loc, state, finh->nodeid, 0, NULL);
        if ((state->loc.inode == NULL) ||
            (ret < 0)) {
        ...
        if (ret >= 0) {
                ...
                if (state->name) {
                        ...
                        if (value_data) {
                                ...
                                /* if(ret >...)...else if...else */
                        }  else if (!strcmp (state->name, "user.lwfs-booster-volfile")) {
                                if (!priv->volfile) {
                                ...
                                /* if(ret >...)...else if...else */
                        } else if (!strcmp (state->name, "user.lwfs-booster-path")) {
                                send_fuse_xattr (this, finh, state->loc.path,
                                                 strlen (state->loc.path) + 1, state->size);
                        } else if (!strcmp (state->name, "user.lwfs-booster-mount")) {
...
#ifndef IOCTL /* wanghy add */
static int
fuse_ioctl_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
                int32_t op_ret, int32_t op_errno, uint32_t cmd, int64_t retaddr)
{
        fuse_state_t *state = NULL;
        fuse_in_header_t *finh = NULL;
        ...
        if (op_ret < 0) {
                ...
                switch(cmd){
                        case LL_IOC_LOV_SETSTRIPE:
                                send_fuse_err (this, finh, op_ret);
                                break;
			case LL_IOC_LOV_GETSTRIPE:
                        case IOC_MDC_GETFILESTRIPE:
                                gf_log ("lwfs-fuse", GF_LOG_DEBUG,
                                        "WANGHY enter fuse_ioctl_cbk (case IOC_MDC_GETFILESTRIPE)");
                                ...
                                send_fuse_data(this, finh, (struct lov_user_md *)retaddr, lum_size);
                                break;
                        /* hexb@20100505 */
                        case LWFS_IOC_SETOP:
                                send_fuse_data(this, finh, retaddr, sizeof(int));
                                break;
                        case LWFS_IOC_GETOP:
                                send_fuse_data(this, finh, retaddr, sizeof(int));
                                break;
...
fuse_ioctl (xlator_t *this, fuse_in_header_t *finh, void *msg)
{
        struct fuse_ioctl_in *fri = msg;
        void *arg = (void *)(fri+1);
        fuse_state_t *state = NULL;
        fd_t         *fd = NULL;
        int ret = -1;
	int level; /* hexb@20100505 */

        GET_STATE (this, finh, state);
        state->cmd = fri->cmd;
        state->size = fri->in_size;
        fd = FH_TO_FD (fri->fh);
        state->fd = fd;

        switch (state->cmd) {
                case LL_IOC_LOV_SETSTRIPE:
                        if(fri->in_size == sizeof(struct lov_user_md_v1)){
                                struct lov_user_md_v1 *lum = (struct lov_user_md_v1 *)arg;
                        	FUSE_FOP (state, fuse_ioctl_cbk, GF_FOP_IOCTL,
                                	  ioctl, fd, fri->cmd, lum);
                        }
                        if(fri->in_size == sizeof(struct lov_user_md_v3)){
                                struct lov_user_md_v3 *lum = (struct lov_user_md_v3 *)arg;
                        	FUSE_FOP (state, fuse_ioctl_cbk, GF_FOP_IOCTL,
                                	  ioctl, fd, fri->cmd, lum);
                        }
                        break;
		case LL_IOC_LOV_GETSTRIPE:
                case IOC_MDC_GETFILESTRIPE:
                        state->name=strdup((char *)arg);
                        FUSE_FOP (state, fuse_ioctl_cbk, GF_FOP_IOCTL,
                                  ioctl, fd, state->cmd, state->name);
                        break;
                /* hexb@20100505 */
                case LWFS_IOC_SETDB:
                        if (fri->in_size == sizeof(int)) {
                                int *newl = (int *) arg;
                                if ((*newl > GF_LOG_TRACE) || (*newl < GF_LOG_NONE)) {
                                        gf_log("lwfs-fuse", GF_LOG_ERROR,
                                                "set loglevel failed because of bad level value %d",
                                                *newl);
                                        send_fuse_err(this, finh, EFAULT);
                                } else {
                                        gf_log_set_loglevel(*newl);
                                        gf_log("lwfs-fuse", GF_LOG_TRACE,
                                                "set loglevel success to %d",
                                                *newl);
                                        send_fuse_err(this, finh, 0);
                                }
                        }
                        ...
                        break;
                case LWFS_IOC_GETDB:
                        level = (int) gf_log_get_loglevel();;
                        send_fuse_data(this, finh, &level, sizeof(int));
                        free_state(state);
                        break;
                case LWFS_IOC_SETOP:
                        if (fri->in_size == sizeof(int)) {
                                FUSE_FOP (state, fuse_ioctl_cbk, GF_FOP_IOCTL,
                                                 ioctl, fd, state->cmd, arg);
                        }
                        break;
                case LWFS_IOC_GETOP:
                        if (fri->in_size == sizeof(int)) {
...
fuse_thread_proc (void *data)
{
        ...
        pthread_mutex_lock (&priv->child_up_mutex);
        {
			...
			if ( ret == 0 )  {
				...
				if ( i%10000 == 0 ){
  					gf_log (this->name, GF_LOG_DEBUG,
					" pthread_cond_timedout  ret: %d childup: %d",
					ret, priv->child_up_value);
				}
        			pthread_mutex_lock (&priv->child_up_mutex);
			}
#else
        ...
        for (;;) {
                ...
                if (res == -1) {
                        if (errno == ENODEV || errno == EBADF) {
                                gf_log ("lwfs-fuse", GF_LOG_NORMAL,
                ...
                if (res < sizeof (finh)) {
                        gf_log ("lwfs-fuse", GF_LOG_WARNING, "short read on /dev/fuse");
                        break;
                }

                finh = (fuse_in_header_t *)iov_in[0].iov_base;
                if (res != finh->len) {
                        gf_log ("lwfs-fuse", GF_LOG_WARNING, "inconsistent read on /dev/fuse");
                        break;
                }
...
init (xlator_t *this_xl)
{
...
#ifndef WEIWEI_20160516
        if (ret != 0)
                priv->attribute_timeout = 0.0; /* default */
#else
        if (ret != 0)
        ...
        if (!priv->entry_timeout)
                priv->entry_timeout = 0.0; /* default */
#else
        if (!priv->entry_timeout)
...
#ifndef behe_110926_acl
	priv->acl_support = 0;
	ret = dict_get_str (options, "acl-support", &value_string);
	if (value_string) {
		ret = gf_string2boolean(value_string, &priv->acl_support);
	}
#endif

        fsname = this_xl->ctx->cmd_args.volume_file;
        fsname = (fsname ? fsname : this_xl->ctx->cmd_args.volfile_server);
        fsname = (fsname ? fsname : "lwfs");
...
#ifdef WEIWEI_20160323
        fuse_ops[FUSE_GETLK]       = fuse_enosys;
        fuse_ops[FUSE_SETLK]       = fuse_enosys;
        fuse_ops[FUSE_SETLKW]      = fuse_enosys;
#else
...
#ifdef WEIWEI_20160323
        fuse_ops[FUSE_IOCTL]       = fuse_enosys;
#else
        fuse_ops[FUSE_IOCTL]       = fuse_ioctl;
#endif
        return 0;
