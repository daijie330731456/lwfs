/*
  Copyright (c) 2006, 2007, 2008 LW, Inc. <http://www.lw.com>
...
static int __rw_allowable (pl_inode_t *, posix_lock_t *, lwfs_fop_t);
                       ...
                       dict_t *dict)
{
        ...
        ret = dict_set_int32 (dict, LWFS_ENTRYLK_COUNT, count);
        if (ret < 0) {
                gf_log (this->name, GF_LOG_DEBUG,
                        " dict_set failed on key %s", LWFS_ENTRYLK_COUNT);
        }
                       ...
                       dict_t *dict)
{
        ...
        ret = dict_set_int32 (dict, LWFS_INODELK_COUNT, count);
        if (ret < 0) {
                gf_log (this->name, GF_LOG_DEBUG,
                        " dict_set failed on key %s", LWFS_INODELK_COUNT);
        }
                       ...
                       dict_t *dict)
{
        ...
        ret = dict_set_int32 (dict, LWFS_POSIXLK_COUNT, count);
        if (ret < 0) {
                gf_log (this->name, GF_LOG_DEBUG,
                        " dict_set failed on key %s", LWFS_POSIXLK_COUNT);
        }
		...
		dict_t *xattr_req)
{
        ...
        if (dict_get (xattr_req, LWFS_ENTRYLK_COUNT))
                local->entrylk_count_req = 1;
        if (dict_get (xattr_req, LWFS_INODELK_COUNT))
                local->inodelk_count_req = 1;
        if (dict_get (xattr_req, LWFS_POSIXLK_COUNT))
