...
int lwfs_graph_parent_up (xlator_t *graph);
...
static int 
_fork_and_fetch (lwfs_ctx_t *ctx,
		 FILE *spec_fp,
		 const char *remote_host,
		 const char *transport,
		 uint32_t remote_port)
