...
extern int
gf_log_central_init (lwfs_ctx_t *ctx, const char *remote_host,
...
const char *argp_program_version = "" \
        ...
        "\nRepository revision: " LWFS_REPOSITORY_REVISION "\n"  \
        "Copyright (c) 2006-2009 LW Inc. "             \
        "<http://www.lw.com>\n"                                \
        "LWFS comes with ABSOLUTELY NO WARRANTY.\n"              \
        "You may redistribute copies of LWFS under the terms of "\
...
gf_daemon (int *pipe_fd)
{
        ...
        if (pipe (pipe_fd) < 0) {
                gf_log ("lwfs", GF_LOG_ERROR,
                        "pipe creation error- %s", strerror (errno));
                return -1;
        }

        if ((pid = fork ()) < 0) {
                gf_log ("lwfs", GF_LOG_ERROR, "fork error: %s",
                        strerror (errno));
                return -1;
        } else if (pid != 0) {
                close (pipe_fd[1]);
...
int
lwfs_graph_parent_up (xlator_t *graph)
{
        xlator_t *trav = NULL;
        int       ret = -1;
...
int
lwfs_graph_init (xlator_t *graph, int fuse)
{
        volume_opt_list_t *vol_opt = NULL;
...
cleanup_and_exit (int signum)
{
        ...
        gf_log ("lwfs", GF_LOG_WARNING, "shutting down");
        ...
        if (ctx->graph) {
                ...
                gf_log ("lwfs", GF_LOG_DEBUG, "no graph present");
        }
}
...
#ifdef LIXI_MEMPOOL
mempool_preallocate_list preallocate_list[] = {
  {64, 3},
  {128, 3},
  {256, 3},
  {0, 0}
...
main (int argc, char *argv[])
{
...
#ifdef LIXI_MEMPOOL
			if ((mixed_pool = create_mempool(64, 1048576)) == NULL)
			{
				fprintf(stderr, "Failed to create mempool\n");
				exit(1); 
			}
			mempool_preallocate(&mixed_pool, preallocate_list);
#endif

        ret = lwfs_globals_init ();
        if (ret)
                return ret;
	...
	/* ignore the signal when file size is very big. */
        ...
        ctx->graph = graph;
        if (lwfs_graph_init (graph, fuse_volume_found) != 0) {
                gf_log ("lwfs", GF_LOG_ERROR,
        ...
        lwfs_graph_parent_up (graph);

        gf_log ("lwfs", GF_LOG_NORMAL, "Successfully started");
