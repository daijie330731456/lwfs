...
#ifndef behe_100825
void gf_proc_clean_xlator_itable(xlator_t *this_xl)
{
	char *posix_path;
	int posix_path_len;
	struct posix_private *private;
	xlator_t *travel_xl;
...
#if 0
	while (travel_xl) {
		if (!strcmp(travel_xl->type, "storage/posix")) {
			travel_xl->dumpops->cleanitable(travel_xl, NULL);
			get_posix_path (&posix_path, &posix_path_len);
			gf_proc_dump_write("after get posix_path", "%s", posix_path);
		}
		travel_xl = travel_xl->next;
	}
	travel_xl = this_xl;
#endif
	while (travel_xl) {
		//gf_proc_dump_write("type", "%s", travel_xl->type);
		if (!strcmp(travel_xl->type, "protocol/server")) {
			travel_xl->dumpops->cleanitable(travel_xl, posix_path);
		}
		travel_xl = travel_xl->next;
	}
#endif
...
void gf_proc_clean_itable(int signum)
{
	int          ret = -1;
        lwfs_ctx_t   *ctx = NULL;
	...
	ctx = get_global_ctx_ptr();
	if (ctx) {
		gf_proc_clean_xlator_itable(ctx->graph);
	}
