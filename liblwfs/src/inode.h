...
struct _inode_table {
        ...
        uint32_t           attic_size;
#ifndef JJH
	struct list_head   free_inode;		/* do not really free() the inode, but link to the free_inode list */
	uint32_t	   finode_size;		/* free inodes in freee_inode list */
	/* He Xiaobin add @20100315 to identify inode using fuse_gen but its addr */
	struct list_head  *fuse_hash;  /* buckets for inode fuse hash table */
        uint64_t gen_req;
#endif
};
...
#ifndef WEIWEI
        struct list_head   child_list;         /* child dentry list of par inode */
#endif
};
...
#ifndef WEIWEI
        struct list_head     child_dentry_list;          /* child_dentry list */
        struct list_head     unset_list;          /* unset list */
	/* He Xiaobin add @ 20100315 */
        uint64_t        fuse_gen;
        struct list_head        gen_hash;
#endif
...
#ifndef JJH
void
inode_prealloc(inode_table_t *table, int num);
#endif
