...
static lwfs_ctx_t *gf_global_ctx;
...
gf_global_variable_init()
{
	...
	gf_fop_list[GF_FOP_FGETXATTR]   = "FGETXATTR";
        ...
        gf_fop_list[GF_FOP_IOCTL]   	= "IOCTL";
#endif
...
void
set_global_ctx_ptr (lwfs_ctx_t *ctx)
{
	gf_global_ctx = ctx;
}

/* 
 * Don't use this function other than in lwfsd.c. liblwfsclient does 
 * not set gf_global_ctx since there can be multiple lwfs-contexts 
...
gf_print_trace (int32_t signum)
{
	...
	{
		extern lwfs_ctx_t *gf_global_ctx;
		lwfs_ctx_t *ctx = gf_global_ctx;
...
#ifndef JJH /* don't change initial checksum value, for iovec cksum */
void
