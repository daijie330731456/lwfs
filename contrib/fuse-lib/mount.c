...
#ifdef GF_FUSERMOUNT
#define FUSERMOUNT_PROG FUSERMOUNT_DIR "/fusermount-lwfs"
#else
...
#define GFFUSE_LOGERR(...) \
        gf_log ("lwfs-fuse", GF_LOG_ERROR, ## __VA_ARGS__)
#endif /* !FUSE_UTIL */

/*
 * Functions below, until following note, were taken from libfuse
 * (http://git.lw.com/?p=users/csaba/fuse.git;a=commit;h=b988bbf9)
 * almost verbatim. What has been changed:
 * - style adopted to that of lwfs
 * - s/fprintf/gf_log/
                    ...
                    const char *mnt, const char *type, const char *opts)
{
        ...
        if (res == 0) {
#ifndef WEIWEI
                char templ[] = "/dev/shm/fusermountXXXXXX";
#else
...
fuse_mount_sys (const char *mountpoint, char *fsname, char *mnt_param)
{
        ...
        if (ret == -1 && errno == ENODEV) {
                /* fs subtype support was added by 79c0b2df aka
                   ...
                   older kernel ... */
                fstype = "fuse";
                ret = asprintf (&source, "lwfs#%s", fsname);
                if (ret == -1) {
                        GFFUSE_LOGERR ("Out of memory");
...
gf_fuse_mount (const char *mountpoint, char *fsname, char *mnt_param)
{
        ...
        if (fd == -1) {
                ...
                rv = asprintf (&fm_mnt_params,
                               "%s,fsname=%s,nonempty,subtype=lwfs",
                               mnt_param, fsname);
